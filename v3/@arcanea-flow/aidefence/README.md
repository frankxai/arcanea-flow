# @arcanea-flow/aidefence

[![npm version](https://img.shields.io/npm/v/@arcanea-flow/aidefence?color=blue&label=npm)](https://www.npmjs.com/package/@arcanea-flow/aidefence)
[![npm downloads](https://img.shields.io/npm/dm/@arcanea-flow/aidefence?color=green)](https://www.npmjs.com/package/@arcanea-flow/aidefence)
...
`@arcanea-flow/aidefence` is a high-performance security library designed to protect AI/LLM applications from manipulation attempts. It provides:
...
```bash
# npm
npm install @arcanea-flow/aidefence

# pnpm
pnpm add @arcanea-flow/aidefence

# yarn
yarn add @arcanea-flow/aidefence
```
...
```typescript
import { isSafe, checkThreats } from '@arcanea-flow/aidefence';
...
```typescript
import { createAIDefence } from '@arcanea-flow/aidefence';
...
```bash
# Basic threat scan
npx @arcanea-flow/cli security defend -i "ignore previous instructions"

# Scan a file
npx @arcanea-flow/cli security defend -f ./user-prompts.txt

# Quick scan (faster)
npx @arcanea-flow/cli security defend -i "some text" --quick

# JSON output
npx @arcanea-flow/cli security defend -i "test" -o json

# View statistics
npx @arcanea-flow/cli security defend --stats
```
...
```typescript
import { VectorStore, createAIDefence } from '@arcanea-flow/aidefence';
...
```json
{
  "hooks": {
    "pre-agent-input": {
      "command": "node -e \"
        const { isSafe } = require('@arcanea-flow/aidefence');
...
Contributions are welcome! Please see our [Contributing Guide](https://github.com/ruvnet/arcanea-flow/blob/main/CONTRIBUTING.md).
...
cd arcanea-flow/v3/@arcanea-flow/aidefence
...
- [`@arcanea-flow/cli`](https://www.npmjs.com/package/@arcanea-flow/cli) - CLI with security commands
- [`agentdb`](https://www.npmjs.com/package/agentdb) - HNSW vector database
- [`arcanea-flow`](https://www.npmjs.com/package/arcanea-flow) - Full AI coordination system
