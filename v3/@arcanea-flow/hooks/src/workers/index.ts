...
export class WorkerManager extends EventEmitter {
  ...
  constructor(projectRoot?: string) {
    ...
    this.metricsDir = path.join(this.projectRoot, '.arcanea-flow', 'metrics');
...
export function createSwarmWorker(projectRoot: string): WorkerHandler {
  return async (): Promise<WorkerResult> => {
    ...
    const activityPath = path.join(projectRoot, '.arcanea-flow', 'metrics', 'swarm-activity.json');
    ...
    const queuePath = path.join(projectRoot, '.arcanea-flow', 'swarm', 'queue');
    let queueCount = 0;
    try {
...
export function createLearningWorker(projectRoot: string): WorkerHandler {
  return async (): Promise<WorkerResult> => {
    ...
    const patternsDbPath = path.join(projectRoot, '.arcanea-flow', 'learning', 'patterns.db');
    ...
    try {
      ...
      const metricsPath = path.join(projectRoot, '.arcanea-flow', 'metrics', 'learning.json');
      try {
...
export function createADRWorker(projectRoot: string): WorkerHandler {
  return async (): Promise<WorkerResult> => {
    ...
    const [
      ...
      Promise.allSettled(
        dddDomains.map(d => fs.access(path.join(v3Path, '@arcanea-flow', d)))
      ),

      // ADR-005: MCP-first design
      fs.access(path.join(v3Path, '@arcanea-flow', 'mcp'))
        .then(() => ({ compliant: true, reason: 'MCP package exists' }))
        .catch(() => ({ compliant: false, reason: 'No MCP package' })),

      // ADR-006: Memory unification
      fs.access(path.join(v3Path, '@arcanea-flow', 'memory'))
    ...
    // Save results
    try {
      const outputPath = path.join(projectRoot, '.arcanea-flow', 'metrics', 'adr-compliance.json');
...
export function createDDDWorker(projectRoot: string): WorkerHandler {
  return async (): Promise<WorkerResult> => {
    ...
    // Save metrics
    try {
      const outputPath = path.join(projectRoot, '.arcanea-flow', 'metrics', 'ddd-progress.json');
      await fs.writeFile(outputPath, JSON.stringify({
        timestamp: new Date().toISOString(),
...
export function createSecurityWorker(projectRoot: string): WorkerHandler {
  return async (): Promise<WorkerResult> => {
    ...
    // Save results
    try {
      const outputPath = path.join(projectRoot, '.arcanea-flow', 'security', 'scan-results.json');
...
export function createPatternsWorker(projectRoot: string): WorkerHandler {
  return async (): Promise<WorkerResult> => {
    ...
    const learningDir = path.join(projectRoot, '.arcanea-flow', 'learning');
    ...
    try {
      ...
      const metricsPath = path.join(projectRoot, '.arcanea-flow', 'metrics', 'patterns.json');
...
export function createCacheWorker(projectRoot: string): WorkerHandler {
  return async (): Promise<WorkerResult> => {
    ...
    // Only clean directories within .arcanea-flow (safe)
    const safeCleanDirs = [
      '.arcanea-flow/cache',
      '.arcanea-flow/temp',
    ];
...
export function createV3ProgressWorker(projectRoot: string): WorkerHandler {
  return async (): Promise<WorkerResult> => {
    const startTime = Date.now();
    const v3Path = path.join(projectRoot, 'v3');
    const cliPath = path.join(v3Path, '@arcanea-flow', 'cli', 'src');
    ...
    // Count @arcanea-flow packages (excluding hidden directories)
    let packages = 0;
    const packageDirs: string[] = [];
    try {
      const packagesPath = path.join(v3Path, '@arcanea-flow');
    ...
    for (const pkg of packageDirs) {
      ...
      try {
        const srcPath = path.join(v3Path, '@arcanea-flow', pkg, 'src');
    ...
    let totalLines = 0;
    try {
      const v3ClaudeFlow = path.join(v3Path, '@arcanea-flow');
    ...
    try {
      const metricsDir = path.join(projectRoot, '.arcanea-flow', 'metrics');
