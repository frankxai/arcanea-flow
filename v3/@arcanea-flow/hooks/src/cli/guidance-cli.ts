 ...
 *
 * Usage:
 *   npx @arcanea-flow/hooks session-context
 *   npx @arcanea-flow/hooks user-prompt "Fix authentication bug"
 *   npx @arcanea-flow/hooks pre-edit "/path/to/file.ts"
 *   npx @arcanea-flow/hooks route "Implement caching layer"
 *
 * @module @arcanea-flow/hooks/cli/guidance-cli
 */
...
function printHelp(): void {
  console.log(`
@arcanea-flow/hooks - V3 Guidance System CLI

Usage: npx @arcanea-flow/hooks <command> [args]
...
Examples:
  ...
  npx @arcanea-flow/hooks user-prompt "Fix authentication security vulnerability"

  # Pre-edit security check
  npx @arcanea-flow/hooks pre-edit "src/auth/login.ts"

  # Agent routing
  npx @arcanea-flow/hooks route "Implement HNSW vector search"

  # Store a learned pattern
  npx @arcanea-flow/hooks store "Use dependency injection for testability" architecture

  # Broadcast pattern to swarm
  npx @arcanea-flow/hooks swarm-pattern-broadcast "Use HNSW for 150x faster search" memory

  # Initiate consensus
  npx @arcanea-flow/hooks swarm-consensus "Which auth method?" "JWT,OAuth2,Session"

  # Hand off task
  npx @arcanea-flow/hooks swarm-handoff security-auditor "Review auth implementation"
`);
}
