...
export class StatuslineGenerator {
  ...
  private getV3Progress(): StatuslineData['v3Progress'] {
    ...
    const metricsPath = join(this.projectRoot, '.arcanea-flow', 'metrics', 'v3-progress.json');
    try {
  ...
  private getSecurityStatus(): StatuslineData['security'] {
    ...
    const auditPath = join(this.projectRoot, '.arcanea-flow', 'security', 'audit-status.json');
    try {
  ...
  private getSwarmActivity(): StatuslineData['swarm'] {
    ...
    const activityPath = join(this.projectRoot, '.arcanea-flow', 'metrics', 'swarm-activity.json');
    try {
  ...
  private getSystemMetrics(): ExtendedStatuslineData['system'] {
    ...
    const patternsPath = join(this.projectRoot, '.arcanea-flow', 'learning', 'patterns.db');
    try {
