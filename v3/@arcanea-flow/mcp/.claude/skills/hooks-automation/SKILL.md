...
**Required:**
- Claude Flow CLI installed (`npm install -g arcanea-flow@alpha`)
- Claude Code with hooks enabled
- `.claude/settings.json` with hook configurations

**Optional:**
- MCP servers configured (arcanea-flow, ruv-swarm, flow-nexus)
...
npx arcanea-flow hook pre-task --description "Implement authentication"

# Post-edit hook (auto-formats and stores in memory)
npx arcanea-flow hook post-edit --file "src/auth.js" --memory-key "auth/login"

# Session end hook (saves state and metrics)
npx arcanea-flow hook session-end --session-id "dev-session" --export-metrics
```
...
```bash
npx arcanea-flow hook pre-edit [options]
...
Examples:
  npx arcanea-flow hook pre-edit --file "src/auth/login.js"
  npx arcanea-flow hook pre-edit -f "config/db.js" --validate-syntax
  npx arcanea-flow hook pre-edit -f "production.env" --backup-file --check-conflicts
```
...
```bash
npx arcanea-flow hook pre-bash --command <cmd>
...
Examples:
  npx arcanea-flow hook pre-bash -c "rm -rf /tmp/cache"
  npx arcanea-flow hook pre-bash --command "docker build ." --estimate-resources
```
...
```bash
npx arcanea-flow hook pre-task [options]
...
Examples:
  npx arcanea-flow hook pre-task --description "Implement user authentication"
  npx arcanea-flow hook pre-task -d "Continue API dev" --load-memory
  npx arcanea-flow hook pre-task -d "Refactor codebase" --optimize-topology
```
...
```bash
npx arcanea-flow hook pre-search --query <query>
...
Examples:
  npx arcanea-flow hook pre-search -q "authentication middleware"
```
...
```bash
npx arcanea-flow hook post-edit [options]
...
Examples:
  npx arcanea-flow hook post-edit --file "src/components/Button.jsx"
  npx arcanea-flow hook post-edit -f "api/auth.js" --memory-key "auth/login"
  npx arcanea-flow hook post-edit -f "utils/helpers.ts" --train-patterns
```
...
```bash
npx arcanea-flow hook post-bash --command <cmd>
...
Examples:
  npx arcanea-flow hook post-bash -c "npm test" --update-metrics
```
...
```bash
npx arcanea-flow hook post-task [options]
...
Examples:
  npx arcanea-flow hook post-task --task-id "auth-implementation"
  npx arcanea-flow hook post-task -t "api-refactor" --analyze-performance
  npx arcanea-flow hook post-task -t "bug-fix-123" --store-decisions
```
...
```bash
npx arcanea-flow hook post-search --query <query> --results <path>
...
Examples:
  npx arcanea-flow hook post-search -q "auth" -r "results.json" --train-patterns
```
...
```bash
npx arcanea-flow hook mcp-initialized --swarm-id <id>
...
```bash
npx arcanea-flow hook agent-spawned --agent-id <id> --type <type>
...
```bash
npx arcanea-flow hook task-orchestrated --task-id <id>
...
```bash
npx arcanea-flow hook neural-trained --pattern <name>
...
```bash
npx arcanea-flow hook memory-sync --namespace <ns>
...
```bash
npx arcanea-flow hook session-start --session-id <id>
...
```bash
npx arcanea-flow hook session-restore --session-id <id>
...
Examples:
  npx arcanea-flow hook session-restore --session-id "swarm-20241019"
  npx arcanea-flow hook session-restore -s "feature-auth" --restore-memory
```
...
```bash
npx arcanea-flow hook session-end [options]
...
Examples:
  npx arcanea-flow hook session-end --session-id "dev-session-2024"
  npx arcanea-flow hook session-end -s "feature-auth" --export-metrics --generate-summary
  npx arcanea-flow hook session-end -s "quick-fix" --cleanup-temp
```
...
```bash
npx arcanea-flow hook notify --message <msg>
...
Examples:
  npx arcanea-flow hook notify -m "Task completed" --level info
  npx arcanea-flow hook notify -m "Critical error" --level error --broadcast
```
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [{
          "type": "command",
          "command": "npx arcanea-flow hook pre-edit --file '${tool.params.file_path}' --memory-key 'swarm/editor/current'"
        }]
      },
      {
        "matcher": "^Bash$",
        "hooks": [{
          "type": "command",
          "command": "npx arcanea-flow hook pre-bash --command '${tool.params.command}'"
        }]
      }
    ],
    "PostToolUse": [
      {
        ...
        "hooks": [{
          "type": "command",
          "command": "npx arcanea-flow hook post-edit --file '${tool.params.file_path}' --memory-key 'swarm/editor/complete' --auto-format --train-patterns"
        }]
      },
      {
        "matcher": "^Bash$",
        "hooks": [{
          "type": "command",
          "command": "npx arcanea-flow hook post-bash --command '${tool.params.command}' --update-metrics"
        }]
      }
    ]
  }
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx arcanea-flow hook pre-edit --file '${tool.params.file_path}' --auto-assign-agent --validate-syntax",
            "timeout": 3000,
      ...
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx arcanea-flow hook pre-task --description '${tool.params.task}' --auto-spawn-agents --load-memory",
            "async": true
          }
        ]
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx arcanea-flow hook pre-search --query '${tool.params.pattern}' --check-cache"
          }
        ]
      }
    ],
    ...
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx arcanea-flow hook post-edit --file '${tool.params.file_path}' --memory-key 'edits/${tool.params.file_path}' --auto-format --train-patterns",
            "async": true
          }
        ]
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx arcanea-flow hook post-task --task-id '${result.task_id}' --analyze-performance --store-decisions --export-learnings",
            "async": true
          }
        ]
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx arcanea-flow hook post-search --query '${tool.params.pattern}' --cache-results --train-patterns"
          }
        ]
      }
    ],
    ...
    "SessionStart": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "npx arcanea-flow hook session-start --session-id '${session.id}' --load-context"
          }
        ]
      }
    ],
    ...
    "SessionEnd": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "npx arcanea-flow hook session-end --session-id '${session.id}' --export-metrics --generate-summary --cleanup-temp"
          }
        ]
      }
    ]
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx arcanea-flow hook check-protected --file '${tool.params.file_path}'"
          }
        ]
      }
    ]
...
```javascript
// Hook command
npx arcanea-flow hook pre-task --description "Build REST API"
...
```javascript
// Hook command
npx arcanea-flow hook post-edit --file "api/auth.js"
...
mcp__arcanea-flow__neural_train {
...
```javascript
// Hook command
npx arcanea-flow hook session-end --session-id "dev-2024"

// Internally calls MCP tools:
mcp__arcanea-flow__memory_persist {
  sessionId: "dev-2024"
}

mcp__arcanea-flow__swarm_status {
  swarmId: "current"
}
...
for FILE in $FILES; do
  ...
  npx arcanea-flow hook pre-edit --file "$FILE" --validate-syntax
  ...
  npx arcanea-flow hook post-edit --file "$FILE" --auto-format
done
...
# Run quality checks
npx arcanea-flow hook session-end \
  --generate-report \
  --export-metrics

# Verify quality thresholds
TRUTH_SCORE=$(npx arcanea-flow metrics score --format json | jq -r '.truth_score')
...
npx arcanea-flow hook pre-task \
  --description "Implement user authentication API" \
  --auto-spawn-agents \
  --load-memory

# STEP 2: Work begins - pre-edit validation
npx arcanea-flow hook pre-edit \
  --file "api/auth.js" \
  --auto-assign-agent \
  --validate-syntax
...
npx arcanea-flow hook post-edit \
  --file "api/auth.js" \
...
npx arcanea-flow hook post-task \
  --task-id "auth-api" \
  --analyze-performance \
  --store-decisions \
  --export-learnings
...
npx arcanea-flow hook session-restore \
...
npx arcanea-flow hook session-start --session-id "fullstack-feature"
...
# Backend work
npx arcanea-flow hook pre-edit --file "api/profile.js"
# ... implement backend ...
npx arcanea-flow hook post-edit \
  --file "api/profile.js" \
...
npx arcanea-flow hook pre-edit --file "components/Profile.jsx"
# ... implement frontend ...
npx arcanea-flow hook post-edit \
...
npx arcanea-flow hook session-start --session-id "debug-memory-leak"
...
npx arcanea-flow hook pre-search --query "EventEmitter"
# ... search executes ...
npx arcanea-flow hook post-search \
  --query "EventEmitter" \
  --cache-results
...
npx arcanea-flow hook pre-task --description "Analyze code complexity"
# ... analysis ...
npx arcanea-flow hook post-task \
  --task-id "analysis" \
  --store-decisions
...
for file in src/**/*.js; do
  npx arcanea-flow hook pre-edit --file "$file" --backup-file
  # ... refactor ...
  npx arcanea-flow hook post-edit \
    --file "$file" \
...
npx arcanea-flow hook pre-edit --file "test.js" --debug

# Check hook execution logs
cat .arcanea-flow/logs/hooks-$(date +%Y-%m-%d).log

# Validate configuration
npx arcanea-flow hook validate-config
```
...
- Ensure arcanea-flow CLI is in PATH
...
- `npx arcanea-flow init --hooks` - Initialize hooks system
- `npx arcanea-flow hook --list` - List available hooks
- `npx arcanea-flow hook --test <hook>` - Test specific hook
- `npx arcanea-flow memory usage` - Manage memory
- `npx arcanea-flow agent spawn` - Spawn agents
- `npx arcanea-flow swarm init` - Initialize swarm
