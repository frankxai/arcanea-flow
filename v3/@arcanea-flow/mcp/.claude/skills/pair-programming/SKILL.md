...
```bash
# Check status
arcanea-flow pair --status

# View history
arcanea-flow pair --history
...
# End session
arcanea-flow pair --end [--save] [--report]
```
...
```bash
arcanea-flow pair --start --mode driver
```
...
```bash
arcanea-flow pair --start --mode navigator
```
...
arcanea-flow pair --start --mode switch

# 5-minute intervals (rapid)
arcanea-flow pair --start --mode switch --interval 5m

# 15-minute intervals (deep focus)
arcanea-flow pair --start --mode switch --interval 15m
```
...
#### Basic Configuration
Create `.arcanea-flow/pair-config.json`:
...
```bash
# Set configuration
arcanea-flow pair config set defaultMode switch
arcanea-flow pair config set verification.threshold 0.98

# Get configuration
arcanea-flow pair config get
arcanea-flow pair config get defaultMode

# Export/Import
arcanea-flow pair config export > config.json
arcanea-flow pair config import config.json

# Reset
arcanea-flow pair config reset
```
...
```bash
# Create profile
arcanea-flow pair profile create refactoring \
  --mode driver \
  --verify true \
  --threshold 0.98 \
  --focus refactor
...
# Use profile
arcanea-flow pair --start --profile refactoring

# List profiles
arcanea-flow pair profile list
```
...
arcanea-flow pair --template refactor
...
# Feature template
arcanea-flow pair --template feature
...
# Debug template
arcanea-flow pair --template debug
...
# Learning template
arcanea-flow pair --template learn
# Mode: Mentor
# Pace: Slow
# Explanations: Detailed
# Examples: Many
...
```bash
# Save session
arcanea-flow pair --save [--name <name>]

# Load session
arcanea-flow pair --load <session-id>

# Export session
arcanea-flow pair --export <session-id> [--format json|md]

# Generate report
arcanea-flow pair --report <session-id>
```
...
```bash
# Start in background
arcanea-flow pair --start --background

# Monitor background session
arcanea-flow pair --monitor

# Attach to background session
arcanea-flow pair --attach <session-id>

# End background session
arcanea-flow pair --end <session-id>
```
...
arcanea-flow pair --start --record

# Replay session
arcanea-flow pair --replay <session-id>

# Session analytics
arcanea-flow pair --analytics <session-id>
```
...
**With Git:**
```bash
arcanea-flow pair --start --git --auto-commit
```

**With CI/CD:**
```bash
arcanea-flow pair --start --ci --non-interactive
```

**With IDE:**
```bash
arcanea-flow pair --start --ide vscode
```
...
- Run `arcanea-flow pair config validate`
...
- `arcanea-flow pair --help` - Show help
- `arcanea-flow pair config` - Manage configuration
- `arcanea-flow pair profile` - Manage profiles
- `arcanea-flow pair templates` - List templates
- `arcanea-flow pair agents` - List available agents
