...
npx arcanea-flow sparc run integration "connect payment service"

# For alpha features
npx arcanea-flow@alpha sparc run integration "connect payment service"

# With namespace
npx arcanea-flow sparc run integration "your task" --namespace integration

# Non-interactive mode
npx arcanea-flow sparc run integration "your task" --non-interactive
```

### Option 3: Local Installation
```bash
# If arcanea-flow is installed locally
./arcanea-flow sparc run integration "connect payment service"
```
...
```bash
# Store mode-specific context
npx arcanea-flow memory store "integration_context" "important decisions" --namespace integration

# Query previous work
npx arcanea-flow memory query "integration" --limit 5
```
