...
## Basic Usage
```bash
./arcanea-flow swarm "your complex task" --strategy <type> [options]
```
...
```bash
./arcanea-flow swarm "Build e-commerce REST API" \
...
```bash
./arcanea-flow swarm "Analyze AI market trends 2024-2025" \
  --strategy research \
  --background \
  --distributed \
  --max-agents 8
...
```bash
./arcanea-flow swarm "Optimize database queries and API performance" \
...
```bash
./arcanea-flow swarm "Implement secure payment processing system" \
  --strategy development \
  --mode distributed \
  --max-agents 10 \
  --parallel \
...
```bash
./arcanea-flow swarm "Comprehensive security audit and testing" \
  --strategy testing \
  --review \
  --verbose \
  --max-agents 6
...
./arcanea-flow monitor

# Monitor specific component
./arcanea-flow monitor --focus swarm
```
...
./arcanea-flow status --verbose
```

### View agent activity:
```bash
# List all agents
./arcanea-flow agent list

# Agent details
./arcanea-flow agent info <agent-id>
```
...
./arcanea-flow memory store "swarm_objective" "Build scalable API" --namespace swarm

# Query swarm progress
./arcanea-flow memory query "swarm_progress" --namespace swarm

# Export swarm memory
./arcanea-flow memory export swarm-results.json --namespace swarm
```
...
### Dry run to preview:
```bash
./arcanea-flow swarm "Test task" --dry-run --strategy development
```

### Custom quality thresholds:
```bash
./arcanea-flow swarm "High quality API" \
  --strategy development \
