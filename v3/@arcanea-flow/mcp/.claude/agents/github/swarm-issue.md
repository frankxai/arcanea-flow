...
npx arcanea-flow@v3alpha github issue-to-swarm 456 \
...
npx arcanea-flow@v3alpha github issues-batch \
  --issues "$ISSUES" \
  --parallel
...
npx arcanea-flow@v3alpha github issue-analyze 456 \
  --suggest-agents \
  --estimate-complexity \
  --create-subtasks
```
...
# Initialize swarm
npx arcanea-flow@v3alpha github issue-init 456 \
  --issue-data "$ISSUE" \
...
SUBTASKS=$(npx arcanea-flow@v3alpha github issue-decompose 456 \
  --body "$ISSUE_BODY" \
  --max-subtasks 10 \
  --assign-priorities)
...
# Get swarm progress
PROGRESS=$(npx arcanea-flow@v3alpha github issue-progress 456)

# Update checklist in issue body
UPDATED_BODY=$(echo "$CURRENT" | jq -r '.body' | \
  npx arcanea-flow@v3alpha github update-checklist --progress "$PROGRESS")
...
npx arcanea-flow@v3alpha github issue-deps 456 \
  --resolve-order \
  --parallel-safe \
  --update-blocking
```
...
npx arcanea-flow@v3alpha github epic-swarm \
  --epic 123 \
...
npx arcanea-flow@v3alpha github create-issues \
  --from-analysis \
  --template "bug-report" \
  --auto-assign
```
        ...
        with:
          command: |
            if [[ "${{ github.event.label.name }}" == "swarm-ready" ]]; then
              npx arcanea-flow@v3alpha github issue-init ${{ github.event.issue.number }}
            fi
```
...
npx arcanea-flow@v3alpha github issue-board-sync \
  --project "Development" \
  --column-mapping '{
    "To Do": "pending",
    "In Progress": "active",
...
npx arcanea-flow@v3alpha github bug-swarm 456 \
  --reproduce \
  --isolate \
  --fix \
  --test
...
npx arcanea-flow@v3alpha github feature-swarm 456 \
  --design \
  --implement \
  --document \
  --demo
...
```bash
# Refactoring swarm
npx arcanea-flow@v3alpha github debt-swarm 456 \
  --analyze-impact \
  --plan-migration \
  --execute \
  --validate
...
echo "$STALE_ISSUES" | jq -r '.number' | while read -r num; do
  ...
  # Analyze with swarm
  ACTION=$(npx arcanea-flow@v3alpha github analyze-stale \
    --issue "$ISSUE" \
    --suggest-action)
...
npx arcanea-flow@v3alpha github triage \
  --unlabeled \
  --analyze-content \
  --suggest-labels \
  --assign-priority
...
npx arcanea-flow@v3alpha github find-duplicates \
  --threshold 0.8 \
  --link-related \
  --close-duplicates
```
...
npx arcanea-flow@v3alpha github link-pr \
  --issue 456 \
  --pr 789 \
  --update-both
```
...
npx arcanea-flow@v3alpha github milestone-swarm \
  --milestone "v2.0" \
  --parallel-issues \
  --track-progress
```
...
npx arcanea-flow@v3alpha github cross-repo \
  --issue "org/repo#456" \
...
npx arcanea-flow@v3alpha github issue-metrics \
  --issue 456 \
...
npx arcanea-flow@v3alpha github effectiveness \
...
npx arcanea-flow@v3alpha github issue-init 789 \
...
npx arcanea-flow@v3alpha github issue-init 234 \
  --topology mesh \
...
npx arcanea-flow@v3alpha github issue-init 567 \
  --topology ring \
...
mcp__arcanea-flow__agent_spawn { type: "coordinator", name: "Issue Coordinator" }
mcp__arcanea-flow__agent_spawn { type: "analyst", name: "Issue Analyzer" }
mcp__arcanea-flow__agent_spawn { type: "coder", name: "Solution Developer" }
mcp__arcanea-flow__agent_spawn { type: "tester", name: "Validation Engineer" }
