...
npx arcanea-flow benchmark-run --suite comprehensive --duration 300

# Execute specific benchmark
npx arcanea-flow benchmark-run --suite throughput --iterations 10

# Compare with baseline
npx arcanea-flow benchmark-compare --current <results> --baseline <baseline>

# Quality assessment
npx arcanea-flow quality-assess --target swarm-performance --criteria throughput,latency

# Performance validation
npx arcanea-flow validate-performance --results <file> --criteria <file>
```

### Regression Detection Commands
```bash
# Detect performance regressions
npx arcanea-flow detect-regression --current <results> --historical <data>

# Set up automated regression monitoring
npx arcanea-flow regression-monitor --enable --sensitivity 0.95

# Analyze error patterns
npx arcanea-flow error-analysis --logs <log-files>
```
