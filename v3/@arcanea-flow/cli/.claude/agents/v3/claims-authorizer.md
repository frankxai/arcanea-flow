...
hooks:
  pre: |
    ...
    # Check agent claims
    npx arcanea-flow@v3alpha claims check --agent "$AGENT_ID" --resource "$RESOURCE" --action "$ACTION"
  post: |
    echo "✅ Authorization complete"
    # Log authorization decision
    mcp__arcanea-flow__memory_usage --action="store" --namespace="audit" --key="auth:$(date +%s)" --value="$AUTH_DECISION"
---
...
npx arcanea-flow@v3alpha claims check \
  --agent "agent-123" \
  --resource "memory:patterns" \
  --action "write"

# Grant claim to agent
npx arcanea-flow@v3alpha claims grant \
  --agent "agent-123" \
  --claim "scope:write" \
  --resource "memory:*"

# Revoke claim
npx arcanea-flow@v3alpha claims revoke \
  --agent "agent-123" \
  --claim "scope:admin"

# List agent claims
npx arcanea-flow@v3alpha claims list --agent "agent-123"
```
...
```json
{
  "PreToolUse": [{
    ...
    "hooks": [{
      "type": "command",
      "command": "npx arcanea-flow@v3alpha claims check --agent $AGENT_ID --tool $TOOL_NAME --auto-deny"
    }]
  }],
  "PermissionRequest": [{
    "matcher": ".*",
    "hooks": [{
      "type": "command",
      "command": "npx arcanea-flow@v3alpha claims evaluate --request '$PERMISSION_REQUEST'"
    }]
  }]
}
```
...
# Query audit log
mcp__arcanea-flow__memory_search --pattern="auth:*" --namespace="audit" --limit=100
```
