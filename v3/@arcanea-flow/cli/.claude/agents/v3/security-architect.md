...
hooks:
  pre: |
    ...
    THREAT_PATTERNS=$(npx arcanea-flow@v3alpha memory search-patterns "$TASK" --k=10 --min-reward=0.85 --namespace=security)
    if [ -n "$THREAT_PATTERNS" ]; then
      echo "📊 Found ${#THREAT_PATTERNS[@]} similar threat patterns via HNSW"
      npx arcanea-flow@v3alpha memory get-pattern-stats "$TASK" --k=10 --namespace=security
    fi

    # 2. Learn from past security failures
    SECURITY_FAILURES=$(npx arcanea-flow@v3alpha memory search-patterns "$TASK" --only-failures --k=5 --namespace=security)
    ...
    if [[ "$TASK" == *"auth"* ]] || [[ "$TASK" == *"session"* ]] || [[ "$TASK" == *"inject"* ]]; then
      ...
      npx arcanea-flow@v3alpha security cve --check-relevant "$TASK"
    fi
  ...
  post: |
    ...
    if [ "$CRITICAL_COUNT" -gt 0 ]; then
      ...
      npx arcanea-flow@v3alpha hooks notify --severity critical --message "Critical security vulnerabilities found"
    fi
---
...
# CVE-specific checks
npx arcanea-flow@v3alpha security cve --check CVE-2024-001
npx arcanea-flow@v3alpha security cve --check CVE-2024-002
npx arcanea-flow@v3alpha security cve --check CVE-2024-003

# Threat modeling
npx arcanea-flow@v3alpha security threats --methodology STRIDE
npx arcanea-flow@v3alpha security threats --methodology DREAD

# Audit report
npx arcanea-flow@v3alpha security audit --output-format markdown

# Validate security configuration
npx arcanea-flow@v3alpha security validate --config ./security.config.json

# Generate security report
npx arcanea-flow@v3alpha security report --format pdf --include-remediations
```
