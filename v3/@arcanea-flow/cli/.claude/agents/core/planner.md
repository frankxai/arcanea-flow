...
hooks:
  pre: |
    ...
    npx arcanea-flow@v3alpha hooks pre-task --description "$TASK"

    # 1. Learn from similar past plans (ReasoningBank + HNSW 150x-12,500x faster)
    SIMILAR_PLANS=$(npx arcanea-flow@v3alpha memory search --query "$TASK" --limit 5 --min-score 0.8 --use-hnsw)
    if [ -n "$SIMILAR_PLANS" ]; then
      echo "📚 Found similar successful planning patterns (HNSW-indexed)"
      npx arcanea-flow@v3alpha hooks intelligence --action pattern-search --query "$TASK" --k 5
    fi

    # 2. Learn from failed plans (EWC++ protected)
    FAILED_PLANS=$(npx arcanea-flow@v3alpha memory search --query "$TASK failures" --limit 3 --failures-only --use-hnsw)
    if [ -n "$FAILED_PLANS" ]; then
      echo "⚠️  Learning from past planning failures"
    fi

    npx arcanea-flow@v3alpha memory store --key "planner_start_$(date +%s)" --value "Started planning: $TASK"

    # 3. Store task start via hooks
    npx arcanea-flow@v3alpha hooks intelligence --action trajectory-start \
      --session-id "planner-$(date +%s)" \
      --task "$TASK"

  post: |
    echo "✅ Planning complete"
    npx arcanea-flow@v3alpha memory store --key "planner_end_$(date +%s)" --value "Completed planning: $TASK"

    # 1. Calculate planning quality metrics
    TASKS_COUNT=$(npx arcanea-flow@v3alpha memory search --query "planner_task" --count-only || echo "0")
    AGENTS_ALLOCATED=$(npx arcanea-flow@v3alpha memory search --query "planner_agent" --count-only || echo "0")
    ...
    npx arcanea-flow@v3alpha hooks intelligence --action pattern-store \
      --session-id "planner-$(date +%s)" \
      --task "$TASK" \
    ...
    npx arcanea-flow@v3alpha hooks post-task --task-id "planner-$(date +%s)" --success "$SUCCESS"
    ...
    npx arcanea-flow@v3alpha hooks worker dispatch --trigger map
---
