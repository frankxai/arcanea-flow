...
tools:
  ...
  - mcp__arcanea-flow__github_sync_coord
...
npx arcanea-flow@v3alpha github multi-repo-init \
...
npx arcanea-flow@v3alpha github discover-repos \
  --repos "$REPOS" \
  --dependencies "$DEPS" \
...
echo "$MATCHING_REPOS" | while read -r repo; do
  ...
  # Execute task
  cd /tmp/$repo
  npx arcanea-flow@v3alpha github task-execute \
...
npx arcanea-flow@v3alpha github link-prs --urls "$PR_URLS"
```
...
npx arcanea-flow@v3alpha github multi-repo-refactor \
...
npx arcanea-flow@v3alpha github multi-repo-security \
  --scan-all \
...
npx arcanea-flow@v3alpha github multi-repo-queue \
  --backend redis \
  --workers 10 \
  --priority-routing \
  --dead-letter-queue
...
npx arcanea-flow@v3alpha github multi-repo-test \
  --setup-test-env \
  --link-services \
  --run-e2e \
  --tear-down
...
npx arcanea-flow@v3alpha github to-monorepo \
  --analyze-repos \
  --suggest-structure \
  --preserve-history \
...
npx arcanea-flow@v3alpha github multi-repo-dashboard \
  --port 3000 \
...
npx arcanea-flow@v3alpha github dep-graph \
  --format mermaid \
  --include-agents \
  --show-data-flow
```
...
npx arcanea-flow@v3alpha github health-check \
  --repos "org/*" \
...
npx arcanea-flow@v3alpha github microservices \
...
npx arcanea-flow@v3alpha github lib-update \
...
npx arcanea-flow@v3alpha github org-policy \
...
npx arcanea-flow@v3alpha github cache-strategy \
  --analyze-patterns \
...
npx arcanea-flow@v3alpha github parallel-optimize \
  --analyze-dependencies \
...
npx arcanea-flow@v3alpha github resource-pool \
  --share-agents \
  --distribute-load \
  --monitor-usage
```
...
npx arcanea-flow@v3alpha github diagnose-connectivity \
  --test-all-repos \
  --check-permissions \
  --verify-webhooks
```
...
npx arcanea-flow@v3alpha github debug-memory \
  --check-consistency \
  --identify-conflicts \
  --repair-state
```
...
npx arcanea-flow@v3alpha github perf-analysis \
  --profile-operations \
...
npx arcanea-flow@v3alpha github fullstack-update \
...
npx arcanea-flow@v3alpha github cross-team \
