...
gh pr view 123 --json body,title,labels,files | npx arcanea-flow@v3alpha swarm create-from-pr

# Auto-spawn agents based on PR labels
gh pr view 123 --json labels | npx arcanea-flow@v3alpha swarm auto-spawn

# Create swarm with PR context
gh pr view 123 --json body,labels,author,assignees | \
  npx arcanea-flow@v3alpha swarm init --from-pr-data
```
      ...
      - name: Handle Swarm Command
        run: |
          if [[ "${{ github.event.comment.body }}" == /swarm* ]]; then
            npx arcanea-flow@v3alpha github handle-comment \
...
npx arcanea-flow@v3alpha github pr-topology --pr 123
```
...
npx arcanea-flow@v3alpha github pr-init 123 \
  --auto-agents \
  --pr-data "$PR_INFO" \
  --diff "$PR_DIFF" \
  --analyze-impact
...
PROGRESS=$(npx arcanea-flow@v3alpha github pr-progress 123 --format markdown)
...
# Run swarm review
REVIEW_RESULTS=$(npx arcanea-flow@v3alpha github pr-review 123 \
...
npx arcanea-flow@v3alpha github multi-pr \
  --prs "123,124,125" \
  --strategy "parallel" \
  --share-memory
```
...
npx arcanea-flow@v3alpha github pr-deps 123 \
  --spawn-agents \
  --resolve-conflicts
```
...
```bash
# Auto-fix PR issues
npx arcanea-flow@v3alpha github pr-fix 123 \
...
SWARM_STATUS=$(npx arcanea-flow@v3alpha github pr-status 123)
...
createServer((req, res) => {
  if (req.url === '/github-webhook') {
    ...
    if (event.action === 'opened' && event.pull_request) {
      execSync(`npx arcanea-flow@v3alpha github pr-init ${event.pull_request.number}`);
    }
...
npx arcanea-flow@v3alpha github pr-init 456 \
...
npx arcanea-flow@v3alpha github pr-init 789 \
  --topology mesh \
...
npx arcanea-flow@v3alpha github pr-init 321 \
  --topology ring \
...
npx arcanea-flow@v3alpha github pr-report 123 \
...
npx arcanea-flow@v3alpha github export-metrics \
  --pr 123 \
  --to-insights
```
...
mcp__arcanea-flow__agent_spawn { type: "coordinator", name: "PR Coordinator" }
mcp__arcanea-flow__agent_spawn { type: "reviewer", name: "Code Reviewer" }
mcp__arcanea-flow__agent_spawn { type: "tester", name: "Test Engineer" }
mcp__arcanea-flow__agent_spawn { type: "analyst", name: "Impact Analyzer" }
...
mcp__arcanea-flow__coordination_sync { swarmId: "pr-review-swarm" }
