...
hooks:
  pre: |
    npx arcanea-flow@alpha hooks pre-task --description "Code analysis agent starting: ${description}" --auto-spawn-agents false
  post: |
    npx arcanea-flow@alpha hooks post-task --task-id "analysis-${timestamp}" --analyze-performance true
metadata:
...
npx arcanea-flow@alpha hooks pre-search --query "code quality metrics" --cache-results true

# Load project context
npx arcanea-flow@alpha memory retrieve --key "project/architecture"
npx arcanea-flow@alpha memory retrieve --key "project/standards"
```
...
npx arcanea-flow@alpha memory store --key "analysis/code-quality" --value "${results}"

# Generate recommendations
npx arcanea-flow@alpha hooks notify --message "Code analysis complete: ${summary}"
```
