...
```bash
arcanea-flow pair [--start] [options]
```
...
arcanea-flow pair --start --agent senior-dev

# Start with verification
arcanea-flow pair --start --verify --threshold 0.98
```
...
```bash
arcanea-flow pair --start --mode switch --interval 10m
```
...
arcanea-flow pair --end --save

# End and generate report
arcanea-flow pair --end --report
```
...
arcanea-flow pair --start --git --review-commit
```

### With Testing Frameworks
```bash
# Jest integration
arcanea-flow pair --start --test-framework jest

# Pytest integration
arcanea-flow pair --start --test-framework pytest
```
...
Configure pair programming in `.arcanea-flow/config.json`:
