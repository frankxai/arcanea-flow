...
npx arcanea-flow sparc run mcp "integrate with external API"

# For alpha features
npx arcanea-flow@alpha sparc run mcp "integrate with external API"

# With namespace
npx arcanea-flow sparc run mcp "your task" --namespace mcp

# Non-interactive mode
npx arcanea-flow sparc run mcp "your task" --non-interactive
```

### Option 3: Local Installation
```bash
# If arcanea-flow is installed locally
./arcanea-flow sparc run mcp "integrate with external API"
```
...
```bash
# Store mode-specific context
npx arcanea-flow memory store "mcp_context" "important decisions" --namespace mcp

# Query previous work
npx arcanea-flow memory query "mcp" --limit 5
```
