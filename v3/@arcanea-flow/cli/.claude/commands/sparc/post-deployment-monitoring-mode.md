...
npx arcanea-flow sparc run post-deployment-monitoring-mode "monitor production metrics"

# For alpha features
npx arcanea-flow@alpha sparc run post-deployment-monitoring-mode "monitor production metrics"

# With namespace
npx arcanea-flow sparc run post-deployment-monitoring-mode "your task" --namespace post-deployment-monitoring-mode

# Non-interactive mode
npx arcanea-flow sparc run post-deployment-monitoring-mode "your task" --non-interactive
```

### Option 3: Local Installation
```bash
# If arcanea-flow is installed locally
./arcanea-flow sparc run post-deployment-monitoring-mode "monitor production metrics"
```
...
```bash
# Store mode-specific context
npx arcanea-flow memory store "post-deployment-monitoring-mode_context" "important decisions" --namespace post-deployment-monitoring-mode

# Query previous work
npx arcanea-flow memory query "post-deployment-monitoring-mode" --limit 5
```
