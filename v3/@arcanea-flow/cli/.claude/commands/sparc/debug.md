...
npx arcanea-flow sparc run debug "fix memory leak in service"

# For alpha features
npx arcanea-flow@alpha sparc run debug "fix memory leak in service"

# With namespace
npx arcanea-flow sparc run debug "your task" --namespace debug

# Non-interactive mode
npx arcanea-flow sparc run debug "your task" --non-interactive
```

### Option 3: Local Installation
```bash
# If arcanea-flow is installed locally
./arcanea-flow sparc run debug "fix memory leak in service"
```
...
```bash
# Store mode-specific context
npx arcanea-flow memory store "debug_context" "important decisions" --namespace debug

# Query previous work
npx arcanea-flow memory query "debug" --limit 5
```
