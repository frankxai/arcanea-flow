...
arcanea-flow truth --period 7d

# Export to HTML report
arcanea-flow truth --export report.html --format html
```

### Advanced Analysis
```bash
# Monitor real-time scores
arcanea-flow truth --watch

# Find problematic files
arcanea-flow truth --threshold 0.8

# Agent-specific metrics
arcanea-flow truth --agent coder --period 24h

# JSON for processing
arcanea-flow truth --format json | jq '.overall_score'
```
...
- name: Check Truth Scores
  run: |
    arcanea-flow truth --format json > truth.json
...
arcanea-flow truth --format json | \
...
Set truth display preferences in `.arcanea-flow/config.json`:
