...
## Usage
```bash
npx arcanea-flow neural train [options]
```
...
- **Location**: `.arcanea-flow/neural/patterns.json`
- **Stats**: `.arcanea-flow/neural/stats.json`
...
npx arcanea-flow neural train -p coordination -e 50

# Train with custom learning rate
npx arcanea-flow neural train -p optimization -l 0.005

# Train from file
npx arcanea-flow neural train -d ./training-data.json

# Quick training (10 epochs)
npx arcanea-flow neural train -e 10
```
...
npx arcanea-flow neural patterns --action list

# Search patterns by query
npx arcanea-flow neural patterns --action list -q "error handling"

# Analyze patterns
npx arcanea-flow neural patterns --action analyze -q "coordination"
```
