...
```bash
arcanea-flow stream-chain pipeline refactor --timeout 60
```

### Verbose Test Generation
```bash
arcanea-flow stream-chain pipeline test --verbose
```
...
Define custom pipelines in `.arcanea-flow/config.json`:
