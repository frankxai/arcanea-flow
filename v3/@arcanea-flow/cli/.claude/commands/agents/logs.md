...
```bash
npx arcanea-flow agent logs <agent-id> [options]
```
...
```bash
# View last 50 lines
npx arcanea-flow agent logs coder-lx7m9k2

# View last 100 lines
npx arcanea-flow agent logs coder-lx7m9k2 -n 100

# Follow logs (live)
npx arcanea-flow agent logs coder-lx7m9k2 --follow

# Filter by level
npx arcanea-flow agent logs coder-lx7m9k2 -l error

# Logs since time
npx arcanea-flow agent logs coder-lx7m9k2 --since "1h"
npx arcanea-flow agent logs coder-lx7m9k2 --since "2026-01-08T10:00:00"

# JSON output
npx arcanea-flow agent logs coder-lx7m9k2 --format json
```
...
# Last 30 minutes
npx arcanea-flow agent logs coder-lx7m9k2 --since "30m"

# Specific timestamp
npx arcanea-flow agent logs coder-lx7m9k2 --since "2026-01-08T09:00:00"

# Today's logs
npx arcanea-flow agent logs coder-lx7m9k2 --since "today"
```
...
- `npx arcanea-flow agent status` - Current agent status
- `npx arcanea-flow agent health` - Health monitoring
- `npx arcanea-flow agent metrics` - Performance metrics
