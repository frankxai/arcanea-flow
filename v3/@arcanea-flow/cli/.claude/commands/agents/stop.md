...
```bash
npx arcanea-flow agent stop <agent-id> [options]
npx arcanea-flow agent kill <agent-id> [options]  # Alias
```
...
npx arcanea-flow agent stop coder-lx7m9k2

# Force stop (immediate termination)
npx arcanea-flow agent stop coder-lx7m9k2 --force

# Custom shutdown timeout
npx arcanea-flow agent stop coder-lx7m9k2 --timeout 60

# Using kill alias
npx arcanea-flow agent kill researcher-abc123 -f
```
...
npx arcanea-flow agent list -t coder --format json | \
  jq -r '.agents[].id' | \
  xargs -I {} npx arcanea-flow agent stop {} -f

# Stop all idle agents
npx arcanea-flow agent list -s idle --format json | \
  jq -r '.agents[].id' | \
  xargs -I {} npx arcanea-flow agent stop {}
```

## Related Commands

- `npx arcanea-flow agent list` - Find agent IDs
- `npx arcanea-flow agent status` - Check status before stopping
- `npx arcanea-flow swarm destroy` - Stop all agents in swarm
