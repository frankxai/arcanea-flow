...
npx arcanea-flow agent pool

# Set pool size
npx arcanea-flow agent pool --size 5

# Configure auto-scaling
npx arcanea-flow agent pool --min 2 --max 15 --auto-scale

# Pre-warm agents for fast response
npx arcanea-flow agent pool --warmup

# Disable auto-scaling
npx arcanea-flow agent pool --auto-scale false
```
...
Configure in `.arcanea-flow/config.yaml`:
...
- `npx arcanea-flow agent spawn` - Manual agent spawning
- `npx arcanea-flow agent list` - View active agents
- `npx arcanea-flow swarm scale` - Swarm-level scaling
