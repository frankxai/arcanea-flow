...
npx arcanea-flow agent ls [options]  # Alias
```
...
npx arcanea-flow agent list

# List all agents including inactive
npx arcanea-flow agent list --all

# Filter by type
npx arcanea-flow agent list -t coder

# Filter by status
npx arcanea-flow agent list -s idle

# JSON output for scripting
npx arcanea-flow agent list --format json

# Combined filters
npx arcanea-flow agent list -t researcher -s active
```
