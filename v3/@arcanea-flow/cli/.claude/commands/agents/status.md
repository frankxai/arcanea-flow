...
```bash
npx arcanea-flow agent status <agent-id>
npx arcanea-flow agent status --id <agent-id>
```
...
```bash
# Get status by ID
npx arcanea-flow agent status coder-lx7m9k2

# Using --id flag
npx arcanea-flow agent status --id researcher-abc123

# JSON output
npx arcanea-flow agent status coder-lx7m9k2 --format json
```
...
- `npx arcanea-flow agent metrics` - Aggregate metrics
- `npx arcanea-flow agent health` - Health check
- `npx arcanea-flow agent logs <id>` - Activity logs
