...
```bash
npx arcanea-flow agent metrics [agent-id] [options]
```
...
npx arcanea-flow agent metrics coder-lx7m9k2

# Last hour
npx arcanea-flow agent metrics -p 1h

# Last 7 days
npx arcanea-flow agent metrics --period 7d

# JSON output
npx arcanea-flow agent metrics --format json
```
...
- `npx arcanea-flow agent status` - Individual agent metrics
- `npx arcanea-flow performance benchmark` - Full performance suite
- `npx arcanea-flow status` - System-wide status
