...
```bash
npx arcanea-flow agent health [agent-id] [options]
```
...
npx arcanea-flow agent health

# Specific agent health
npx arcanea-flow agent health coder-lx7m9k2

# Continuous monitoring
npx arcanea-flow agent health --watch

# Custom interval
npx arcanea-flow agent health -w -i 10

# JSON output
npx arcanea-flow agent health --format json
```
...
- `npx arcanea-flow agent status` - Detailed agent info
- `npx arcanea-flow agent metrics` - Performance metrics
- `npx arcanea-flow doctor` - System-wide health
