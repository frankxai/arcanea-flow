...
```bash
npx arcanea-flow swarm init
```

### Mesh topology for research

```bash
npx arcanea-flow swarm init --topology mesh --max-agents 5 --strategy balanced
```

### Hierarchical for development

```bash
npx arcanea-flow swarm init --topology hierarchical --max-agents 10 --strategy parallel --auto-spawn
```

### GitHub-focused swarm

```bash
npx arcanea-flow swarm init --topology star --github --memory
```
