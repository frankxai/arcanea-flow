...
## Usage
```bash
npx arcanea-flow hive-mind spawn <objective> [options]
```
...
## Examples
```bash
npx arcanea-flow hive-mind spawn "Build API"
npx arcanea-flow hive-mind spawn "Research patterns" --queen-type adaptive
npx arcanea-flow hive-mind spawn "Build service" --claude
```
