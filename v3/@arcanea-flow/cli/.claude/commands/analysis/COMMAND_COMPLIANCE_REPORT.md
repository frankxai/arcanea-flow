...
- `mcp__arcanea-flow__*` tools (preferred)
- `npx arcanea-flow` commands (as fallback)
...
- Updated to: `Tool: mcp__arcanea-flow__token_usage` with appropriate parameters
...
**After**:
```
Tool: mcp__arcanea-flow__token_usage
Parameters: {"operation": "session", "timeframe": "24h"}
```

### 2. performance-bottlenecks.md
**Status**: ✅ Compliant (No changes needed)
**Reason**: Already uses proper `mcp__arcanea-flow__task_results` tool format
...
1. **MCP Tool Usage**: All direct tool calls now use `mcp__arcanea-flow__*` format
