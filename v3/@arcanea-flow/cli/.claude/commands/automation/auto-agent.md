...
```bash
npx arcanea-flow auto agent [options]
```
...
```bash
npx arcanea-flow auto agent --task "Build a REST API with authentication"
```

### Constrained spawning

```bash
npx arcanea-flow auto agent -t "Debug performance issue" --max-agents 3
```

### Analysis only

```bash
npx arcanea-flow auto agent -t "Refactor codebase" --no-spawn
```

### Minimal strategy

```bash
npx arcanea-flow auto agent -t "Fix bug in login" -s minimal
```
