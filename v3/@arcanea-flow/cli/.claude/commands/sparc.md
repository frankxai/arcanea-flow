...
```bash
# Run SPARC orchestrator (default)
npx arcanea-flow sparc "build complete authentication system"

# Run a specific mode
npx arcanea-flow sparc run architect "design API structure"
npx arcanea-flow sparc run tdd "implement user service"

# Execute full TDD workflow
npx arcanea-flow sparc tdd "implement user authentication"

# List all modes with details
npx arcanea-flow sparc modes --verbose

# For alpha features
npx arcanea-flow@alpha sparc run <mode> "your task"
```

### Option 3: Local Installation
```bash
# If arcanea-flow is installed locally
./arcanea-flow sparc "build complete authentication system"
./arcanea-flow sparc run architect "design API structure"
```
...
```bash
# Store specifications
npx arcanea-flow memory store "spec_auth" "OAuth2 + JWT requirements" --namespace spec

# Store architectural decisions
./arcanea-flow memory store "arch_api" "RESTful microservices design" --namespace arch

# Query previous work
./arcanea-flow memory query "authentication" --limit 10

# Export project memory
./arcanea-flow memory export sparc-project-backup.json
```
...
./arcanea-flow swarm "Build e-commerce platform" --strategy development --monitor --review

# Background optimization swarm
./arcanea-flow swarm "Optimize system performance" --strategy optimization --background

# Distributed research swarm
./arcanea-flow swarm "Analyze market trends" --strategy research --distributed --ui
```
...
```bash
./arcanea-flow sparc run code "implement API" --non-interactive
./arcanea-flow sparc tdd "user tests" --non-interactive --enable-permissions
```
...
See `/arcanea-flow-help` for all available commands.
