...
// Monitor execution
mcp__arcanea-flow__swarm_monitor {
  swarmId: "current",
  interval: 5000
}
```
...
npx arcanea-flow sparc run <mode> "task description"

# Use alpha features
npx arcanea-flow@alpha sparc run <mode> "task description"

# List all available modes
npx arcanea-flow sparc modes

# Get help for specific mode
npx arcanea-flow sparc help <mode>

# Run with options
npx arcanea-flow sparc run <mode> "task" --parallel --monitor

# Execute TDD workflow
npx arcanea-flow sparc tdd "feature description"

# Batch execution
npx arcanea-flow sparc batch <mode1,mode2,mode3> "task"

# Pipeline execution
npx arcanea-flow sparc pipeline "task description"
```

### Method 3: Local Installation

**Best for**: Projects with local arcanea-flow installation

```bash
# If arcanea-flow is installed locally
./arcanea-flow sparc run <mode> "task description"
```
...
mcp__arcanea-flow__agent_spawn { type: "architect" }
mcp__arcanea-flow__agent_spawn { type: "coder" }
mcp__arcanea-flow__agent_spawn { type: "tester" }
mcp__arcanea-flow__agent_spawn { type: "reviewer" }
```
...
```javascript
mcp__arcanea-flow__workflow_create {
...
[Single Message]:
  mcp__arcanea-flow__agent_spawn { type: "researcher" }
  mcp__arcanea-flow__agent_spawn { type: "coder" }
  mcp__arcanea-flow__agent_spawn { type: "tester" }
  TodoWrite { todos: [8-10 todos] }

// ❌ WRONG: Multiple messages
Message 1: mcp__arcanea-flow__agent_spawn { type: "researcher" }
Message 2: mcp__arcanea-flow__agent_spawn { type: "coder" }
...
```bash
# Before work
npx arcanea-flow@alpha hooks pre-task --description "implement auth"

# During work
npx arcanea-flow@alpha hooks post-edit --file "auth.js"

# After work
npx arcanea-flow@alpha hooks post-task --task-id "task-123"
```
...
npx arcanea-flow sparc run researcher "authentication patterns"

# Step 2: Architecture design
npx arcanea-flow sparc run architect "design auth system"

# Step 3: TDD implementation
npx arcanea-flow sparc tdd "user authentication feature"

# Step 4: Code review
npx arcanea-flow sparc run reviewer "review auth implementation"

# Step 5: Documentation
npx arcanea-flow sparc run documenter "document auth API"
```
...
npx arcanea-flow sparc run analyzer "investigate bug #456"

# Step 2: Debug systematically
npx arcanea-flow sparc run debugger "fix memory leak in service X"

# Step 3: Create tests
npx arcanea-flow sparc run tester "regression tests for bug #456"

# Step 4: Review fix
npx arcanea-flow sparc run reviewer "validate bug fix"
```
...
npx arcanea-flow sparc run analyzer "profile API response times"

# Step 2: Identify bottlenecks
npx arcanea-flow sparc run optimizer "optimize database queries"

# Step 3: Implement improvements
npx arcanea-flow sparc run coder "implement caching layer"

# Step 4: Benchmark results
npx arcanea-flow sparc run tester "performance benchmarks"
```
...
npx arcanea-flow sparc pipeline "e-commerce checkout feature"
...
mcp__arcanea-flow__context_restore {
...
```javascript
// Analyze repository
mcp__arcanea-flow__github_repo_analyze {
  repo: "owner/repo",
  analysis_type: "code_quality"
}

// Manage pull requests
mcp__arcanea-flow__github_pr_manage {
  repo: "owner/repo",
  pr_number: 123,
  action: "review"
}
...
mcp__arcanea-flow__token_usage {
...
- **NPM Package**: https://www.npmjs.com/package/arcanea-flow
...
# Run specific mode
npx arcanea-flow sparc run <mode> "task"

# TDD workflow
npx arcanea-flow sparc tdd "feature"

# Full pipeline
npx arcanea-flow sparc pipeline "task"

# Batch execution
npx arcanea-flow sparc batch <modes> "task"
```
...
```javascript
// Initialize swarm
mcp__arcanea-flow__swarm_init { topology: "hierarchical" }

// Execute mode
mcp__arcanea-flow__sparc_mode { mode: "coder", task_description: "..." }

// Monitor progress
mcp__arcanea-flow__swarm_monitor { interval: 5000 }

// Store in memory
mcp__arcanea-flow__memory_usage { action: "store", key: "...", value: "..." }
```
