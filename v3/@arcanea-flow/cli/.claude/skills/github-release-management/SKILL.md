...
npx arcanea-flow github release-create \
  --version "2.0.0" \
  --build-artifacts \
...
npx arcanea-flow swarm init --topology hierarchical

# Execute complete release pipeline
npx arcanea-flow sparc pipeline "Release v2.0.0 with full validation"
```
...
[Single Message - Swarm Initialization]:
  ...
  mcp__arcanea-flow__agent_spawn { type: "coordinator", name: "Release Director" }
  mcp__arcanea-flow__agent_spawn { type: "coder", name: "Version Manager" }
  mcp__arcanea-flow__agent_spawn { type: "tester", name: "QA Engineer" }
  mcp__arcanea-flow__agent_spawn { type: "reviewer", name: "Release Reviewer" }
  mcp__arcanea-flow__agent_spawn { type: "analyst", name: "Deployment Analyst" }
  mcp__arcanea-flow__agent_spawn { type: "researcher", name: "Compatibility Checker" }
```
...
npx arcanea-flow github changelog \
  --prs "$PRS" \
  --commits "$COMMITS" \
  --from v1.0.0 \
  --to HEAD \
...
npx arcanea-flow github version-suggest \
  --current v1.2.3 \
  --analyze-commits \
  --check-compatibility \
  --suggest-pre-release
...
```bash
# Multi-platform build coordination
npx arcanea-flow github release-build \
...
npx arcanea-flow github release-test \
...
npx arcanea-flow github release-deploy \
...
[Single Message - Multi-Package Release]:
  ...
  mcp__arcanea-flow__swarm_init { topology: "mesh", maxAgents: 8 }

  // Spawn package-specific agents
  Task("Package A Manager", "Coordinate arcanea-flow package release v1.0.72", "coder")
  ...
  Write("packages/arcanea-flow/package.json", "[v1.0.72 content]")
  ...
  Bash("cd packages/arcanea-flow && npm install && npm test")
  Bash("cd packages/ruv-swarm && npm install && npm test")
  Bash("npm run test:integration")

  // Create unified release PR
  Bash(`gh pr create \
    --title "Release: arcanea-flow v1.0.72, ruv-swarm v1.0.12" \
...
npx arcanea-flow github multi-release \
...
[Single Message - Cross-Repo Release]:
  ...
  mcp__arcanea-flow__swarm_init { topology: "star", maxAgents: 6 }
  ...
  mcp__arcanea-flow__swarm_monitor { interval: 5, duration: 300 }
```
...
npx arcanea-flow github emergency-release \
  --issue 789 \
  --severity critical \
...
npx arcanea-flow github release-validate \
  --checks "
    version-conflicts,
...
npx arcanea-flow github compat-test \
...
npx arcanea-flow github performance-test \
  --baseline v1.9.0 \
  --candidate v2.0.0 \
...
npx arcanea-flow github release-monitor \
  --version v2.0.0 \
...
npx arcanea-flow github release-analytics \
  --version v2.0.0 \
  --compare-with v1.9.0 \
...
npx arcanea-flow github rollback-config \
  --triggers '{
    "error-rate": ">5%",
    "latency-p99": ">1000ms",
    "availability": "<99.9%",
...
npx arcanea-flow github release-security \
  --scan-dependencies \
  --check-secrets \
  --audit-permissions \
  --sign-artifacts \
...
npx arcanea-flow github release-compliance \
    ...
    steps:
      ...
      - name: Initialize Release Swarm
        run: |
          ...
          npx arcanea-flow@alpha swarm init --topology hierarchical
      ...
      - name: Generate Release Changelog
        run: |
          ...
          CHANGELOG=$(npx arcanea-flow@alpha github changelog \
      ...
      - name: Build Release Artifacts
        run: |
          ...
          npx arcanea-flow@alpha github release-build \
            --platforms "linux,macos,windows" \
            --architectures "x64,arm64" \
            --parallel
      ...
      - name: Security Scan
        run: |
          ...
          npx arcanea-flow@alpha github release-security \
            --scan-dependencies \
            --check-secrets \
            --sign-artifacts
      ...
      - name: Post-Release Validation
        run: |
          ...
          # Validate deployment
          npx arcanea-flow@alpha github release-validate \
      ...
      - name: Monitor Release
        run: |
          ...
          npx arcanea-flow@alpha github release-monitor \
            --version ${{ github.ref_name }} \
            --duration 1h \
            --alert-on-errors &
```
  ...
  emergency-hotfix:
    ...
    steps:
      ...
      - name: Emergency Release
        run: |
          npx arcanea-flow@alpha github emergency-release \
...
npx arcanea-flow@alpha diagnostic-run \
  --component build \
  --verbose
...
npx arcanea-flow@alpha github compat-test \
...
npx arcanea-flow@alpha github rollback \
  --to-version v1.9.9 \
...
npx arcanea-flow@alpha github release-analytics \
  --version v2.0.0 \
  --identify-issues
```
...
npx arcanea-flow@alpha github version-sync \
...
- Documentation: https://arcanea-flow.dev/docs
