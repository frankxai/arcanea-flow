...
# Force reinitialize
npx arcanea-flow hive-mind init --force

# Custom configuration
npx arcanea-flow hive-mind init --config hive-config.json
```
...
npx arcanea-flow hive-mind spawn "Build microservices architecture"

# Strategic queen type
npx arcanea-flow hive-mind spawn "Research AI patterns" --queen-type strategic

# Tactical queen with max workers
npx arcanea-flow hive-mind spawn "Implement API" --queen-type tactical --max-workers 12

# Adaptive queen with consensus
npx arcanea-flow hive-mind spawn "Optimize system" --queen-type adaptive --consensus byzantine

# Generate Claude Code commands
npx arcanea-flow hive-mind spawn "Build full-stack app" --claude
```
...
npx arcanea-flow hive-mind metrics

# Monitor collective memory
npx arcanea-flow hive-mind memory
```
...
npx arcanea-flow hive-mind sessions

# Pause a session
npx arcanea-flow hive-mind pause <session-id>

# Resume a paused session
npx arcanea-flow hive-mind resume <session-id>

# Stop a running session
npx arcanea-flow hive-mind stop <session-id>
```
...
```bash
npx arcanea-flow hive-mind spawn "Build REST API" --claude
```
...
npx arcanea-flow sparc tdd "User authentication" --hive-mind
...
npx arcanea-flow hive-mind spawn "Analyze repo quality" --objective "owner/repo"

# PR review coordination
npx arcanea-flow hive-mind spawn "Review PR #123" --queen-type tactical
```
...
```bash
npx arcanea-flow hive-mind spawn "Research ML frameworks" --queen-type strategic
```

**Tactical Queens** - For implementation and execution
```bash
npx arcanea-flow hive-mind spawn "Build authentication" --queen-type tactical
```

**Adaptive Queens** - For optimization and dynamic tasks
```bash
npx arcanea-flow hive-mind spawn "Optimize performance" --queen-type adaptive
```
...
npx arcanea-flow hive-mind memory --gc

# Optimize database
npx arcanea-flow hive-mind memory --optimize

# Export and clear
npx arcanea-flow hive-mind memory --export --clear
```
...
npx arcanea-flow hive-mind spawn "..." --consensus weighted

# Or use simple majority
npx arcanea-flow hive-mind spawn "..." --consensus majority
```
...
```bash
# Frontend hive
npx arcanea-flow hive-mind spawn "Build UI" --name frontend-hive

# Backend hive
npx arcanea-flow hive-mind spawn "Build API" --name backend-hive
...
npx arcanea-flow hive-mind export <session-id> --output backup.json

# Import session
npx arcanea-flow hive-mind import backup.json
```
...
npx arcanea-flow hive-mind spawn "Build e-commerce platform" \
...
```bash
# Spawn research hive
npx arcanea-flow hive-mind spawn "Research GraphQL vs REST" \
  --queen-type adaptive \
  --consensus byzantine
...
```bash
# Review coordination
npx arcanea-flow hive-mind spawn "Review PR #456" \
  --queen-type tactical \
  --max-workers 6
...
- [Hive Mind Documentation](https://github.com/ruvnet/arcanea-flow/docs/hive-mind)
- [Collective Intelligence Patterns](https://github.com/ruvnet/arcanea-flow/docs/patterns)
- [Byzantine Consensus](https://github.com/ruvnet/arcanea-flow/docs/consensus)
- [Memory Optimization](https://github.com/ruvnet/arcanea-flow/docs/memory)
