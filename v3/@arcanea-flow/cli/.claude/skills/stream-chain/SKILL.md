...
```bash
arcanea-flow stream-chain pipeline analysis
```
...
```bash
arcanea-flow stream-chain run <prompt1> <prompt2> [...] [options]
```
...
```bash
arcanea-flow stream-chain pipeline <type> [options]
```
...
```bash
arcanea-flow stream-chain pipeline refactor
```
...
```bash
arcanea-flow stream-chain pipeline test
```
...
```bash
arcanea-flow stream-chain pipeline optimize
```
...
```bash
arcanea-flow stream-chain pipeline refactor --timeout 60 --verbose
```

#### Debug Test Generation

```bash
arcanea-flow stream-chain pipeline test --debug
```

#### Comprehensive Optimization

```bash
arcanea-flow stream-chain pipeline optimize --timeout 90 --verbose
```
...
Define reusable pipelines in `.arcanea-flow/config.json`:
...
```bash
arcanea-flow stream-chain pipeline security
arcanea-flow stream-chain pipeline documentation
```
...
arcanea-flow swarm init --topology mesh
...
# Results stored in .arcanea-flow/memory/stream-chain/
```
...
arcanea-flow stream-chain pipeline optimize --debug
...
```bash
arcanea-flow stream-chain run "complex task" --timeout 120
```
...
```bash
arcanea-flow stream-chain run "step 1" "step 2" --debug
```
...
cat .arcanea-flow/config.json | grep -A 10 "streamChain"
```
