...
mcp__flow-nexus__execution_stream_subscribe({
  stream_type: "arcanea-flow-swarm",
...
npx arcanea-flow@alpha hooks pre-task --description "Initialize swarm"

# Post-task metrics export
npx arcanea-flow@alpha hooks post-task --task-id "swarm-execution"
```
...
**Remember**: Flow Nexus provides cloud-based orchestration infrastructure. For local execution and coordination, use the core `arcanea-flow` MCP server alongside Flow Nexus for maximum flexibility.
