---
name: browser
description: Web browser automation with AI-optimized snapshots for arcanea-flow agents
version: 1.0.0
triggers:
  - /browser
  - browse
...
npx @arcanea-flow/cli memory store --namespace browser-patterns --key "login-flow" --value "snapshot->fill->click->wait"

# Retrieve before similar task
npx @arcanea-flow/cli memory search --query "login automation"
```

### Hooks
```bash
# Pre-browse hook (get context)
npx @arcanea-flow/cli hooks pre-edit --file "browser-task.ts"

# Post-browse hook (record success)
npx @arcanea-flow/cli hooks post-task --task-id "browse-1" --success true
```
