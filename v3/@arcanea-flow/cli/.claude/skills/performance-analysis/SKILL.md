...
```bash
npx arcanea-flow bottleneck detect
```

### Generate Performance Report
```bash
npx arcanea-flow analysis performance-report --format html --include-metrics
```

### Analyze and Auto-Fix
```bash
npx arcanea-flow bottleneck detect --fix --threshold 15
```
...
#### Command Syntax
```bash
npx arcanea-flow bottleneck detect [options]
```
...
npx arcanea-flow bottleneck detect --swarm-id swarm-123 -t 24h

# Export detailed analysis
npx arcanea-flow bottleneck detect -t 24h -e bottlenecks.json

# Auto-fix detected issues
npx arcanea-flow bottleneck detect --fix --threshold 15

# Low threshold for sensitive detection
npx arcanea-flow bottleneck detect --threshold 10 --export critical-issues.json
```
...
mcp__arcanea-flow__bottleneck_detect({
  timeRange: "1h",
  threshold: 20,
  autoFix: false
})
...
#### Command Syntax
```bash
npx arcanea-flow analysis performance-report [options]
```
...
npx arcanea-flow analysis performance-report --compare swarm-123 --format markdown

# Custom output with specific sections
npx arcanea-flow analysis performance-report \
...
npx arcanea-flow swarm monitor --interval 5
...
async function analyzePerformance() {
  ...
  const bottlenecks = await runCommand(
    'npx arcanea-flow bottleneck detect --format json'
  );

  // Generate performance report
  const report = await runCommand(
    'npx arcanea-flow analysis performance-report --format json'
  );
...
npx arcanea-flow bottleneck detect --threshold 10

# Check cache performance
npx arcanea-flow cache manage --action stats

# Review memory metrics
npx arcanea-flow memory usage
```

**Slow Task Execution**
```bash
# Identify slow tasks
npx arcanea-flow task status --detailed

# Analyze coordination overhead
npx arcanea-flow bottleneck detect --time-range 1h
...
npx arcanea-flow analysis performance-report --sections metrics

# Review cache strategy
npx arcanea-flow cache manage --action analyze

# Enable cache warming
npx arcanea-flow bottleneck detect --fix
```
...
- `npx arcanea-flow swarm monitor` - Real-time monitoring
- `npx arcanea-flow token usage` - Token optimization analysis
- `npx arcanea-flow cache manage` - Cache optimization
- `npx arcanea-flow agent metrics` - Agent performance metrics
- `npx arcanea-flow task status` - Task execution analysis
