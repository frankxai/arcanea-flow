...
- Claude Flow installed (`npx arcanea-flow@alpha`)
...
npx arcanea-flow@alpha truth

# Run verification check
npx arcanea-flow@alpha verify check

# Verify specific file with custom threshold
npx arcanea-flow@alpha verify check --file src/app.js --threshold 0.98

# Rollback last failed verification
npx arcanea-flow@alpha verify rollback --last-good
```
...
npx arcanea-flow@alpha truth --period 7d

# View scores for specific agent
npx arcanea-flow@alpha truth --agent coder --period 24h

# Find files/tasks below threshold
npx arcanea-flow@alpha truth --threshold 0.8
```

**Output Formats:**
```bash
# Table format (default)
npx arcanea-flow@alpha truth --format table

# JSON for programmatic access
npx arcanea-flow@alpha truth --format json

# CSV for spreadsheet analysis
npx arcanea-flow@alpha truth --format csv

# HTML report with visualizations
npx arcanea-flow@alpha truth --format html --export report.html
```

**Real-time Monitoring:**
```bash
# Watch mode with live updates
npx arcanea-flow@alpha truth --watch

# Export metrics automatically
npx arcanea-flow@alpha truth --export .arcanea-flow/metrics/truth-$(date +%Y%m%d).json
```
...
```bash
# Verify single file
npx arcanea-flow@alpha verify check --file src/app.js

# Verify directory recursively
npx arcanea-flow@alpha verify check --directory src/

# Verify with auto-fix enabled
npx arcanea-flow@alpha verify check --file src/utils.js --auto-fix
...
npx arcanea-flow@alpha verify check --task task-123

# Verify with custom threshold
npx arcanea-flow@alpha verify check --task task-456 --threshold 0.99

# Verbose output for debugging
npx arcanea-flow@alpha verify check --task task-789 --verbose
```

**Batch Verification:**
```bash
# Verify multiple files in parallel
npx arcanea-flow@alpha verify batch --files "*.js" --parallel

# Verify with pattern matching
npx arcanea-flow@alpha verify batch --pattern "src/**/*.ts"

# Integration test suite
npx arcanea-flow@alpha verify integration --test-suite full
```
...
npx arcanea-flow@alpha verify check --json > verification.json
...
npx arcanea-flow@alpha verify rollback --to-commit abc123

# Interactive rollback with preview
npx arcanea-flow@alpha verify rollback --interactive
```

**Smart Rollback:**
```bash
# Rollback only failed files (preserve good changes)
npx arcanea-flow@alpha verify rollback --selective

# Rollback with automatic backup
npx arcanea-flow@alpha verify rollback --backup-first

# Dry-run mode (preview without executing)
npx arcanea-flow@alpha verify rollback --dry-run
```
...
```bash
# JSON report
npx arcanea-flow@alpha verify report --format json

# HTML report with charts
npx arcanea-flow@alpha verify report --export metrics.html --format html

# CSV for data analysis
npx arcanea-flow@alpha verify report --format csv --export metrics.csv

# Markdown summary
npx arcanea-flow@alpha verify report --format markdown
```

**Time-based Reports:**
```bash
# Last 24 hours
npx arcanea-flow@alpha verify report --period 24h

# Last 7 days
npx arcanea-flow@alpha verify report --period 7d

# Last 30 days with trends
npx arcanea-flow@alpha verify report --period 30d --include-trends

# Custom date range
npx arcanea-flow@alpha verify report --from 2025-01-01 --to 2025-01-31
```
...
npx arcanea-flow@alpha verify dashboard

# Custom port
npx arcanea-flow@alpha verify dashboard --port 8080

# Export dashboard data
npx arcanea-flow@alpha verify dashboard --export

# Dashboard with auto-refresh
npx arcanea-flow@alpha verify dashboard --refresh 5s
```
...
Set verification preferences in `.arcanea-flow/config.json`:
...
```json
{
  ...
  "truth": {
    ...
    "autoExport": {
      "enabled": true,
      "path": ".arcanea-flow/metrics/truth-daily.json"
    }
  }
}
```
...
npx arcanea-flow@alpha verify check --threshold 0.99

# Lenient mode (90% acceptable)
npx arcanea-flow@alpha verify check --threshold 0.90

# Set default threshold
npx arcanea-flow@alpha config set verification.threshold 0.98
```
...
verify:
  stage: test
  script:
    - npx arcanea-flow@alpha verify check --threshold 0.95 --json > verification.json
    - |
      score=$(jq '.overallScore' verification.json)
...
npx arcanea-flow@alpha swarm --verify --threshold 0.98

# Hive Mind with auto-rollback
npx arcanea-flow@alpha hive-mind --verify --rollback-on-fail

# Training pipeline with verification
npx arcanea-flow@alpha train --verify --threshold 0.99
```
...
npx arcanea-flow@alpha pair --verify --real-time

# Pair with custom threshold
npx arcanea-flow@alpha pair --verify --threshold 0.97 --auto-fix
```
...
npx arcanea-flow@alpha verify watch --directory src/

# Watch with auto-fix
npx arcanea-flow@alpha verify watch --directory src/ --auto-fix

# Watch with notifications
npx arcanea-flow@alpha verify watch --notify --threshold 0.95
```
...
npx arcanea-flow@alpha truth --format json | \
  curl -X POST https://pushgateway.example.com/metrics/job/arcanea-flow \
  -d @-
...
npx arcanea-flow@alpha verify install-hook --pre-commit

# .git/hooks/pre-commit example:
#!/bin/bash
npx arcanea-flow@alpha verify check --threshold 0.95 --json > /tmp/verify.json

score=$(jq '.overallScore' /tmp/verify.json)
if (( $(echo "$score < 0.95" | bc -l) )); then
  echo "❌ Verification failed with score: $score"
  echo "Run 'npx arcanea-flow@alpha verify check --verbose' for details"
  exit 1
fi
...
npx arcanea-flow@alpha truth --verbose --threshold 0.0

# Check specific criteria
npx arcanea-flow@alpha verify check --verbose

# View agent-specific issues
npx arcanea-flow@alpha truth --agent <agent-name> --format json
```
...
npx arcanea-flow@alpha verify rollback --history
...
```bash
# Increase timeout
npx arcanea-flow@alpha verify check --timeout 60s

# Verify in batches
npx arcanea-flow@alpha verify batch --batch-size 10
```
...
- `npx arcanea-flow@alpha pair` - Collaborative development with verification
- `npx arcanea-flow@alpha train` - Training with verification feedback
- `npx arcanea-flow@alpha swarm` - Multi-agent coordination with quality checks
- `npx arcanea-flow@alpha report` - Generate comprehensive project reports
