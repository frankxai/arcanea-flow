 ...
 * Modernized CLI for Arcanea Flow V3
 *
...
function normalizeHelpCopy(text: string, cliName = 'arcanea-flow'): string {
  return text
    .replace(/@claude-flow\/cli/g, '@arcanea-flow/cli')
    .replace(/\bclaude-flow\b/g, cliName)
    .replace(/Claude Flow/g, 'Arcanea Flow');
}
...
export class CLI {
  ...
  constructor(options: CLIOptions = {}) {
    this.name = options.name || 'arcanea-flow';
    this.description = options.description || 'Arcanea Flow V3 - AI Agent Orchestration Platform';
  ...
  private showHelp(): void {
    ...
    this.output.writeln(this.output.dim('Run "arcanea-flow <command> --help" for command help'));
    this.output.writeln();
    this.output.writeln(this.output.dim('Built for the Arcanea swarm and agent ecosystem'));
    this.output.writeln();
  }
  ...
  private async showCommandHelp(commandName: string): Promise<void> {
    ...
    this.output.writeln(this.output.bold(`${this.name} ${command.name}`));
        this.output.writeln(normalizeHelpCopy(command.description, this.name));
    this.output.writeln();
    ...
    if (command.subcommands && command.subcommands.length > 0) {
      ...
      for (const sub of command.subcommands) {
        ...
        const aliases = sub.aliases ? this.output.dim(` (${sub.aliases.join(', ')})`) : '';
                this.output.writeln(`  ${this.output.highlight(name)} ${normalizeHelpCopy(sub.description, this.name)}${aliases}`);
      }
      this.output.writeln();
    }
    ...
    if (command.examples && command.examples.length > 0) {
      ...
      for (const example of command.examples) {
                this.output.writeln(`  ${this.output.dim('$')} ${normalizeHelpCopy(example.command, this.name)}`);
                this.output.writeln(`    ${this.output.dim(normalizeHelpCopy(example.description, this.name))}`);
      }
      this.output.writeln();
    }
  }
    ...
    try {
      ...
      if (result.checked && result.updatesAvailable.length > 0) {
        ...
        if (nonAutoUpdates.length > 0) {
          ...
          this.output.writeln(
            this.output.dim(`Run 'arcanea-flow update check' for details`)
          );
        }
      }
    } catch {
  ...
  private async loadConfig(configPath?: string): Promise<V3Config | undefined> {
    try {
      ...
      const { loadConfig: loadSystemConfig } = await import('@arcanea-flow/shared');
