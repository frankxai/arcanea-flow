...
export async function autoInstallPackage(
  ...
  if (!validPackageName.test(packageName)) {
    if (!silent) {
      console.error(`[arcanea-flow] Invalid package name: ${packageName}`);
    }
    return false;
  }
  ...
  try {
    if (!silent) {
      console.error(`[arcanea-flow] Auto-installing ${packageName}...`);
    }
    ...
    if (!silent) {
      console.error(`[arcanea-flow] Successfully installed ${packageName}`);
    }
    return true;
  } catch (error) {
    if (!silent) {
      console.error(`[arcanea-flow] Failed to auto-install ${packageName}: ${error}`);
    }
    return false;
  }
}
  ...
  try {
    ...
    if (installed) {
      try {
        ...
        console.error(`[arcanea-flow] ${packageName} installed but failed to load. Restart MCP server.`);
        return null;
      }
    }
    return null;
