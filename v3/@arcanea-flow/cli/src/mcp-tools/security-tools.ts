...
type AIDefenceInstance = ReturnType<typeof import('@arcanea-flow/aidefence').createAIDefence>;
...
async function getAIDefence(): Promise<AIDefenceInstance> {
  ...
  const packageName = '@arcanea-flow/aidefence';
  ...
  if (installAttempted) {
    throw new Error('AIDefence package not available. Install with: npm install @arcanea-flow/aidefence');
  }
  installAttempted = true;

  // Second attempt - auto-install and retry
  console.error(`[arcanea-flow] ${packageName} not found, attempting auto-install...`);
  ...
  try {
    ...
    aidefenceInstance = instance;
    console.error(`[arcanea-flow] ${packageName} loaded successfully after install`);
    return instance;
  } catch (retryError) {
...
const aidefenceIsSafeTool: MCPTool = {
  ...
  handler: async (args: Record<string, unknown>): Promise<MCPToolResult> => {
    ...
    try {
      const { isSafe } = await import('@arcanea-flow/aidefence');
