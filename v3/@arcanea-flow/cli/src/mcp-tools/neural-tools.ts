 ...
 * - Uses @arcanea-flow/embeddings for REAL embeddings when available
 * - Falls back to simulated embeddings when @arcanea-flow/embeddings not installed
 * - Pattern storage and search with cosine similarity
 * - Training progress tracked (actual model training requires external tools)
 *
 * Note: For production neural features, use @arcanea-flow/neural module
 */
...
// Try to import real embeddings from @arcanea-flow/embeddings
let realEmbeddings: { embed: (text: string) => Promise<number[]> } | null = null;
let embeddingServiceName: string = 'none';
try {
  // Dynamic import to avoid hard dependency
  const embeddingsModule = await import('@arcanea-flow/embeddings');
  ...
  // @arcanea-flow/embeddings not available, will use fallback
}

// Storage paths
const STORAGE_DIR = '.arcanea-flow';
  ...
  },
  {
    ...
    handler: async (input) => {
      ...
      return {
        ...
        embeddingProvider: realEmbeddings ? `@arcanea-flow/embeddings (${embeddingServiceName})` : 'hash-based (deterministic)',
        models: {
          total: models.length,
