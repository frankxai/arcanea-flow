...
const DEFAULT_CONFIG: ContainerPoolConfig = {
  ...
  statePath: '.arcanea-flow/container-pool',
...
export class ContainerWorkerPool extends EventEmitter {
  ...
  private async createContainer(): Promise<ContainerInfo | null> {
    ...
    const name = `arcanea-flow-worker-${id}`;
    ...
    try {
      ...
      const args = [
        ...
        '-v', `${join(this.projectRoot, this.config.statePath)}:/root/.arcanea-flow`,
  ...
  private buildWorkerCommand(options: ContainerExecutionOptions): string[] {
    // Use npx to run arcanea-flow daemon trigger
    return [
      'npx', 'arcanea-flow@v3alpha',
      'daemon', 'trigger',
