...
export class WorkerDaemon extends EventEmitter {
  ...
  constructor(projectRoot: string, config?: Partial<DaemonConfig>) {
    ...
    const claudeFlowDir = join(projectRoot, '.arcanea-flow');
  ...
  private async runMapWorker(): Promise<unknown> {
    ...
    const metricsFile = join(this.projectRoot, '.arcanea-flow', 'metrics', 'codebase-map.json');
    const metricsDir = join(this.projectRoot, '.arcanea-flow', 'metrics');
    ...
    const map = {
      ...
      structure: {
        ...
        hasClaudeFlow: existsSync(join(this.projectRoot, '.arcanea-flow')),
      },
      scannedAt: Date.now(),
    };
  ...
  private async runAuditWorkerLocal(): Promise<unknown> {
    ...
    const auditFile = join(this.projectRoot, '.arcanea-flow', 'metrics', 'security-audit.json');
    const metricsDir = join(this.projectRoot, '.arcanea-flow', 'metrics');
  ...
  private async runOptimizeWorkerLocal(): Promise<unknown> {
    ...
    const optimizeFile = join(this.projectRoot, '.arcanea-flow', 'metrics', 'performance.json');
    const metricsDir = join(this.projectRoot, '.arcanea-flow', 'metrics');
  ...
  private async runConsolidateWorker(): Promise<unknown> {
    ...
    const consolidateFile = join(this.projectRoot, '.arcanea-flow', 'metrics', 'consolidation.json');
    const metricsDir = join(this.projectRoot, '.arcanea-flow', 'metrics');
  ...
  private async runTestGapsWorkerLocal(): Promise<unknown> {
    ...
    const testGapsFile = join(this.projectRoot, '.arcanea-flow', 'metrics', 'test-gaps.json');
    const metricsDir = join(this.projectRoot, '.arcanea-flow', 'metrics');
  ...
  private async runBenchmarkWorkerLocal(): Promise<unknown> {
    const benchmarkFile = join(this.projectRoot, '.arcanea-flow', 'metrics', 'benchmark.json');
    const metricsDir = join(this.projectRoot, '.arcanea-flow', 'metrics');
