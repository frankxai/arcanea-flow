...
async function main() {
  ...
  try {
    const similar = findSimilarPlugins(registry, '@arcanea-flow/neural', 3);
    console.log(`  ✅ Found ${similar.length} similar plugins to @arcanea-flow/neural`);
    passed++;
  } catch (e) {
    console.log(`  ❌ Failed: ${(e as Error).message}`);
    failed++;
  ...
  if (failed === 0) {
    ...
    console.log('    npx @arcanea-flow/cli plugins list');
    console.log('    npx @arcanea-flow/cli plugins list --official');
    console.log('    npx @arcanea-flow/cli plugins search -q "plugin creator"');
    console.log('    npx @arcanea-flow/cli plugins info -n plugin-creator');
    console.log('');
  }
