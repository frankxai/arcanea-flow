...
export class PatternDiscovery {
  ...
  async fetchRegistry(cid: string, gateway: string): Promise<PatternRegistry | null> {
    ...
    try {
      ...
      if (hasGCSCredentials()) {
        const gcsUri = `gs://arcanea-flow-patterns/registry/${cid}.json`;
  ...
  private getGenesisRegistry(cid: string): PatternRegistry {
    return {
      ...
      patterns: [
        {
          ...
          publicKey: 'ed25519:arcanea-flow-team-key',
        },
      ],
      ...
      registryPublicKey: 'ed25519:arcanea-flow-registry-key',
    };
  }
