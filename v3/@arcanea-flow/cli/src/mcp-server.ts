 ...
 *
 * @module @arcanea-flow/cli/mcp-server
 * @version 3.0.0
 */
...
const DEFAULT_OPTIONS: Required<MCPServerOptions> = {
  ...
  host: 'localhost',
  port: 3000,
  pidFile: path.join(os.tmpdir(), 'arcanea-flow-mcp.pid'),
  logFile: path.join(os.tmpdir(), 'arcanea-flow-mcp.log'),
  tools: 'all',
  daemonize: false,
  timeout: 30000,
};
...
export class MCPServerManager extends EventEmitter {
  ...
  private async startStdioServer(): Promise<void> {
    ...
    // Log to stderr to not corrupt stdout
      console.error(
        `[${new Date().toISOString()}] INFO [arcanea-flow-mcp] (${sessionId}) Starting in stdio mode`
      );
      ...
      for (const line of lines) {
        if (line.trim()) {
          try {
            ...
            console.error(
              `[${new Date().toISOString()}] ERROR [arcanea-flow-mcp] Failed to parse message:`,
    ...
    process.stdin.on('end', () => {
      console.error(
        `[${new Date().toISOString()}] INFO [arcanea-flow-mcp] (${sessionId}) stdin closed, shutting down...`
      );
      process.exit(0);
    });
    ...
    process.on('SIGINT', () => {
      console.error(
        `[${new Date().toISOString()}] INFO [arcanea-flow-mcp] (${sessionId}) Received SIGINT, shutting down...`
      );
      process.exit(0);
    });
    ...
    process.on('SIGTERM', () => {
      console.error(
        `[${new Date().toISOString()}] INFO [arcanea-flow-mcp] (${sessionId}) Received SIGTERM, shutting down...`
      );
      process.exit(0);
    });
    ...
    try {
      switch (message.method) {
        case 'initialize':
          return {
            ...
            result: {
              ...
              serverInfo: { name: 'arcanea-flow', version: '3.0.0' },
              capabilities: {
                tools: { listChanged: true },
                resources: { subscribe: true, listChanged: true },
              },
        ...
        case 'notifications/initialized':
          ...
          console.error(
          `[${new Date().toISOString()}] INFO [arcanea-flow-mcp] (${sessionId}) Client initialized`
          );
          return null;
      ...
      console.error(
        `[${new Date().toISOString()}] ERROR [arcanea-flow-mcp] Error handling ${message.method}:`,
        error
      );
      return {
        jsonrpc: '2.0',
  ...
  private async startHttpServer(): Promise<void> {
    ...
    const { createMCPServer } = await import('@arcanea-flow/mcp');
    ...
    const mcpServer = createMCPServer(
      {
        name: 'Arcanea Flow MCP Server V3',
        version: '3.0.0',
