 ...
 * Uses .arcanea-flow/neural in the current working directory,
 * falling back to home directory
 */
function getDataDir(): string {
  const cwd = process.cwd();
  const localDir = join(cwd, '.arcanea-flow', 'neural');
  const homeDir = join(homedir(), '.arcanea-flow', 'neural');

  // Prefer local directory if .arcanea-flow exists
  if (existsSync(join(cwd, '.arcanea-flow'))) {
    return localDir;
  }
