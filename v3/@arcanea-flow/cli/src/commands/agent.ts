...
const spawnCommand: Command = {
  ...
  examples: [
    { command: 'arcanea-flow agent spawn --type coder --name bot-1', description: 'Spawn a coder agent' },
    { command: 'arcanea-flow agent spawn -t researcher --task "Research React 19"', description: 'Spawn researcher with task' }
  ],
...
const poolCommand: Command = {
  ...
  examples: [
    { command: 'arcanea-flow agent pool --size 5', description: 'Set pool size' },
    { command: 'arcanea-flow agent pool --min 2 --max 15', description: 'Configure auto-scaling' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    try {
      const result = await callMCPTool<{
...
const healthCommand: Command = {
  ...
  examples: [
    { command: 'arcanea-flow agent health', description: 'Show all agents health' },
    { command: 'arcanea-flow agent health -i agent-001 -d', description: 'Detailed health for specific agent' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const logsCommand: Command = {
  ...
  examples: [
    { command: 'arcanea-flow agent logs -i agent-001', description: 'Show agent logs' },
    { command: 'arcanea-flow agent logs -i agent-001 -f', description: 'Follow agent logs' },
    { command: 'arcanea-flow agent logs -l error --since 1h', description: 'Show errors from last hour' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const agentId = ctx.args[0] || ctx.flags.id as string;
...
export const agentCommand: Command = {
  ...
  options: [],
  examples: [
    { command: 'arcanea-flow agent spawn -t coder', description: 'Spawn a coder agent' },
    { command: 'arcanea-flow agent list', description: 'List all agents' },
    { command: 'arcanea-flow agent status agent-001', description: 'Show agent status' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.writeln();
    output.writeln('Usage: arcanea-flow agent <subcommand> [options]');
    output.writeln();
    ...
    output.writeln();
    output.writeln('Run "arcanea-flow agent <subcommand> --help" for subcommand help');
