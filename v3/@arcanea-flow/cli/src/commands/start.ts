...
function isInitialized(cwd: string): boolean {
  const configPath = path.join(cwd, '.arcanea-flow', 'config.yaml');
...
const startAction = async (ctx: CommandContext): Promise<CommandResult> => {
  ...
  try {
    ...
    if (daemon) {
      ...
      const daemonPidPath = path.join(cwd, '.arcanea-flow', 'daemon.pid');
...
const stopCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      // Clean up daemon PID
      const daemonPidPath = path.join(ctx.cwd, '.arcanea-flow', 'daemon.pid');
      if (fs.existsSync(daemonPidPath)) {
