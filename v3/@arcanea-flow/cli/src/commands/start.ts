 ...
 * System startup for Arcanea Flow orchestration
 */
...
const startAction = async (ctx: CommandContext): Promise<CommandResult> => {
  ...
  if (!isInitialized(cwd)) {
        output.printError('Arcanea Flow is not initialized in this directory');
        output.printInfo('Run "arcanea-flow init" first to initialize');
  ...
  output.writeln();
    output.writeln(output.bold('Starting Arcanea Flow V3'));
  output.writeln();
  ...
  try {
    ...
    // Success output
    output.writeln();
        output.printSuccess('Arcanea Flow V3 is running!');
    output.writeln();
    ...
    output.printList([
            `${output.highlight('arcanea-flow status')} - View system status`,
            `${output.highlight('arcanea-flow agent spawn -t coder')} - Spawn an agent`,
            `${output.highlight('arcanea-flow swarm status')} - View swarm details`,
            `${output.highlight('arcanea-flow stop')} - Stop the system`
    ]);

    // Daemon mode
    if (daemon) {
      output.writeln();
            output.printInfo('Running in daemon mode. Use "arcanea-flow stop" to stop.');
...
const stopCommand: Command = {
  name: 'stop',
    description: 'Stop the Arcanea Flow system',
  options: [
    {
      name: 'force',
      short: 'f',
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.writeln();
        output.writeln(output.bold('Stopping Arcanea Flow'));
    output.writeln();

    if (!force && ctx.interactive) {
      const confirmed = await confirm({
                message: 'Are you sure you want to stop Arcanea Flow?',
        default: false
      });
    ...
    try {
      ...
      output.writeln();
            output.printSuccess('Arcanea Flow stopped successfully');
...
const restartCommand: Command = {
  name: 'restart',
    description: 'Restart the Arcanea Flow system',
  options: [
    {
      name: 'force',
      short: 'f',
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    output.writeln();
        output.writeln(output.bold('Restarting Arcanea Flow'));
    output.writeln();
...
export const startCommand: Command = {
  name: 'start',
  description: 'Start the Arcanea Flow orchestration system',
  ...
  examples: [
        { command: 'arcanea-flow start', description: 'Start with configuration defaults' },
        { command: 'arcanea-flow start --daemon', description: 'Start as background daemon' },
        { command: 'arcanea-flow start --port 3001', description: 'Start MCP on custom port' },
        { command: 'arcanea-flow start --topology mesh', description: 'Start with mesh topology' },
        { command: 'arcanea-flow start --skip-mcp', description: 'Start without MCP server' },
        { command: 'arcanea-flow start quick', description: 'Quick start with defaults' },
        { command: 'arcanea-flow start stop', description: 'Stop the running system' }
  ],
  action: startAction
};
