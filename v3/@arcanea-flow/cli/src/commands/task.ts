...
export const taskCommand: Command = {
  ...
  options: [],
  examples: [
    { command: 'arcanea-flow task create -t implementation -d "Add user auth"', description: 'Create a task' },
    { command: 'arcanea-flow task list', description: 'List pending/running tasks' },
    { command: 'arcanea-flow task list --all', description: 'List all tasks' },
    { command: 'arcanea-flow task status task-123', description: 'Get task details' },
    { command: 'arcanea-flow task cancel task-123', description: 'Cancel a task' },
    { command: 'arcanea-flow task assign task-123 --agent coder-1', description: 'Assign task to agent' },
    { command: 'arcanea-flow task retry task-123', description: 'Retry a failed task' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.writeln();
    output.writeln('Usage: arcanea-flow task <subcommand> [options]');
    output.writeln();
    ...
    output.writeln();
    output.writeln('Run "arcanea-flow task <subcommand> --help" for subcommand help');
