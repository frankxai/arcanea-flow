 ...
 *
 * @module @arcanea-flow/cli/commands/mcp
 * @version 3.0.0
 */
...
const startCommand: Command = {
  ...
  examples: [
    { command: 'arcanea-flow mcp start', description: 'Start with defaults (stdio)' },
    { command: 'arcanea-flow mcp start -p 8080 -t http', description: 'Start HTTP server' },
    { command: 'arcanea-flow mcp start -d', description: 'Start as daemon' },
    { command: 'arcanea-flow mcp start -f', description: 'Force restart (kill existing)' }
  ],
    ...
    if (existingStatus.running) {
      ...
      if (!shouldForceRestart) {
        ...
        if (health.healthy) {
          ...
          output.writeln(output.dim('Use "arcanea-flow mcp stop" to stop the server first, or use --force'));
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    try {
      ...
      if (!status.running) {
        ...
        output.writeln();
        output.writeln(output.dim('Run "arcanea-flow mcp start" to start the server'));
        return { success: true, data: status };
      }
...
const execCommand: Command = {
  ...
  examples: [
    { command: 'arcanea-flow mcp exec -t swarm/init -p \'{"topology":"mesh"}\'', description: 'Execute tool' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
export const mcpCommand: Command = {
  ...
  options: [],
  examples: [
    { command: 'arcanea-flow mcp start', description: 'Start MCP server' },
    { command: 'arcanea-flow mcp start -t http -p 8080', description: 'Start HTTP server on port 8080' },
    { command: 'arcanea-flow mcp status', description: 'Show server status' },
    { command: 'arcanea-flow mcp tools', description: 'List tools' },
    { command: 'arcanea-flow mcp stop', description: 'Stop the server' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold('MCP Server Management'));
    output.writeln();
    output.writeln('Usage: arcanea-flow mcp <subcommand> [options]');
    output.writeln();
