...
const deployCommand: Command = {
  ...
  examples: [
    { command: 'arcanea-flow deployment deploy -e prod', description: 'Deploy to production' },
    { command: 'arcanea-flow deployment deploy --dry-run', description: 'Simulate deployment' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.printBox([
      ...
      ``,
      `URL: https://${env === 'prod' ? 'api' : env}.arcanea-flow.io`,
...
const statusCommand: Command = {
  ...
  examples: [
    { command: 'arcanea-flow deployment status', description: 'Show all environments' },
    { command: 'arcanea-flow deployment status -e prod', description: 'Check production' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    output.writeln();
...
const rollbackCommand: Command = {
  ...
  examples: [
    { command: 'arcanea-flow deployment rollback -e prod', description: 'Rollback production' },
    { command: 'arcanea-flow deployment rollback -e prod -v v3.0.0', description: 'Rollback to specific version' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const historyCommand: Command = {
  ...
  examples: [
    { command: 'arcanea-flow deployment history', description: 'Show all history' },
    { command: 'arcanea-flow deployment history -e prod', description: 'Production history' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const env = ctx.flags.env as string;
...
const environmentsCommand: Command = {
  ...
  examples: [
    { command: 'arcanea-flow deployment environments', description: 'List environments' },
    { command: 'arcanea-flow deployment envs -a create -n preview', description: 'Create environment' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    output.writeln();
    ...
    output.printTable({
      ...
      data: [
        { name: 'production', url: 'https://api.arcanea-flow.io', auto: output.error('No'), protected: output.success('Yes') },
        { name: 'staging', url: 'https://staging.arcanea-flow.io', auto: output.success('Yes'), protected: output.error('No') },
        { name: 'development', url: 'https://dev.arcanea-flow.io', auto: output.success('Yes'), protected: output.error('No') },
        { name: 'preview/*', url: 'https://pr-*.arcanea-flow.io', auto: output.success('Yes'), protected: output.error('No') },
      ],
    });
...
const logsCommand: Command = {
  ...
  examples: [
    { command: 'arcanea-flow deployment logs -e prod', description: 'View production logs' },
    { command: 'arcanea-flow deployment logs -d dep-123', description: 'View specific deployment' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const env = ctx.flags.env as string || 'staging';
...
export const deploymentCommand: Command = {
  ...
  examples: [
    { command: 'arcanea-flow deployment deploy -e prod', description: 'Deploy to production' },
    { command: 'arcanea-flow deployment status', description: 'Check all environments' },
    { command: 'arcanea-flow deployment rollback -e prod', description: 'Rollback production' },
  ],
