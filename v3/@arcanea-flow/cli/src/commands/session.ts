  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      if (result.sessions.length === 0) {
        ...
        output.printInfo('Run "arcanea-flow session save" to create a session');
        return { success: true, data: result };
      }
...
const saveCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      output.printInfo(`Restore with: arcanea-flow session restore ${result.sessionId}`);
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    try {
      ...
      if (error instanceof MCPClientError) {
        ...
        output.printInfo('Start a session with "arcanea-flow start"');
...
export const sessionCommand: Command = {
  ...
  options: [],
  examples: [
    { command: 'arcanea-flow session list', description: 'List all sessions' },
    { command: 'arcanea-flow session save -n "checkpoint-1"', description: 'Save current session' },
    { command: 'arcanea-flow session restore session-123', description: 'Restore a session' },
    { command: 'arcanea-flow session delete session-123', description: 'Delete a session' },
    { command: 'arcanea-flow session export -o backup.json', description: 'Export session to file' },
    { command: 'arcanea-flow session import backup.json', description: 'Import session from file' },
    { command: 'arcanea-flow session current', description: 'Show current session' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.writeln();
    output.writeln('Usage: arcanea-flow session <subcommand> [options]');
    output.writeln();
    ...
    output.writeln();
    output.writeln('Run "arcanea-flow session <subcommand> --help" for subcommand help');
