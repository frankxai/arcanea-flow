...
const initCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.writeln();
    output.printInfo('Initializing Arcanea Flow configuration...');
    output.writeln();
    ...
    output.writeln(output.dim('  Creating arcanea-flow.config.json...'));
    output.writeln(output.dim('  Creating .arcanea-flow/ directory...'));
    ...
    output.writeln(output.dim('  Config file: ./arcanea-flow.config.json'));
...
const getCommand: Command = {
  ...
  examples: [
    { command: 'arcanea-flow config get swarm.topology', description: 'Get swarm topology' },
    { command: 'arcanea-flow config get -k memory.backend', description: 'Get memory backend' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const setCommand: Command = {
  ...
  examples: [
    { command: 'arcanea-flow config set swarm.maxAgents 20', description: 'Set max agents' },
    { command: 'arcanea-flow config set -k memory.backend -v agentdb', description: 'Set memory backend' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const key = ctx.flags.key as string || ctx.args[0];
...
const exportCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const outputPath = ctx.flags.output as string || './arcanea-flow.config.export.json';
...
export const configCommand: Command = {
  ...
  options: [],
  examples: [
    { command: 'arcanea-flow config init --v3', description: 'Initialize V3 config' },
    { command: 'arcanea-flow config get swarm.topology', description: 'Get config value' },
    { command: 'arcanea-flow config set swarm.maxAgents 20', description: 'Set config value' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold('Configuration Management'));
    output.writeln();
    output.writeln('Usage: arcanea-flow config <subcommand> [options]');
    output.writeln();
