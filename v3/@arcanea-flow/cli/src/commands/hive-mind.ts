...
1️⃣ **COLLECTIVE INTELLIGENCE**
   mcp__arcanea-flow__hive-mind_consensus    - Democratic decision making
   mcp__arcanea-flow__hive-mind_memory       - Share knowledge across the hive
   mcp__arcanea-flow__hive-mind_broadcast    - Broadcast to all workers
   mcp__arcanea-flow__neural_patterns        - Neural pattern recognition

2️⃣ **QUEEN COORDINATION**
   mcp__arcanea-flow__hive-mind_status       - Monitor swarm health
   mcp__arcanea-flow__task_create            - Create and delegate tasks
   mcp__arcanea-flow__task_orchestrate       - Orchestrate task distribution
   mcp__arcanea-flow__agent_spawn            - Spawn additional workers

3️⃣ **WORKER MANAGEMENT**
   mcp__arcanea-flow__agent_list             - List all active agents
   mcp__arcanea-flow__agent_status           - Check agent status
   mcp__arcanea-flow__agent_metrics          - Track worker performance
   mcp__arcanea-flow__hive-mind_join         - Add agent to hive
   mcp__arcanea-flow__hive-mind_leave        - Remove agent from hive

4️⃣ **TASK ORCHESTRATION**
   mcp__arcanea-flow__task_create            - Create hierarchical tasks
   mcp__arcanea-flow__task_status            - Track task progress
   mcp__arcanea-flow__task_complete          - Mark tasks complete
   mcp__arcanea-flow__workflow_create        - Create workflows

5️⃣ **MEMORY & LEARNING**
   mcp__arcanea-flow__memory_store           - Store collective knowledge
   mcp__arcanea-flow__memory_retrieve        - Access shared memory
   mcp__arcanea-flow__memory_search          - Search memory patterns
   mcp__arcanea-flow__neural_train           - Learn from experiences
   mcp__arcanea-flow__hooks_intelligence_pattern-store - Store patterns
...
💡 COORDINATION TIPS:
• Use mcp__arcanea-flow__hive-mind_broadcast for swarm-wide announcements
• Check worker status regularly with mcp__arcanea-flow__hive-mind_status
...
const initCommand: Command = {
  ...
  examples: [
    { command: 'arcanea-flow hive-mind init -t hierarchical-mesh', description: 'Init hierarchical mesh' },
    { command: 'arcanea-flow hive-mind init -c byzantine -m 20', description: 'Init with Byzantine consensus' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      output.writeln(output.dim('  Use "arcanea-flow hive-mind spawn" to add workers'));
      output.writeln(output.dim('  Use "arcanea-flow hive-mind spawn --claude" to launch Claude Code'));
...
const spawnCommand: Command = {
  ...
  examples: [
    { command: 'arcanea-flow hive-mind spawn -n 5', description: 'Spawn 5 workers' },
    { command: 'arcanea-flow hive-mind spawn -n 3 -r specialist', description: 'Spawn 3 specialists' },
    { command: 'arcanea-flow hive-mind spawn -t coder -p my-coder', description: 'Spawn coder with custom prefix' },
    { command: 'arcanea-flow hive-mind spawn --claude -o "Build a REST API"', description: 'Launch Claude Code with objective' },
    { command: 'arcanea-flow hive-mind spawn -n 5 --claude -o "Research AI patterns"', description: 'Spawn workers and launch Claude Code' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      if (workerData.length === 0) {
        output.printInfo('No workers in hive. Use "arcanea-flow hive-mind spawn" to add workers.');
      } else {
        output.printTable({
          columns: [
...
const taskCommand: Command = {
  ...
  examples: [
    { command: 'arcanea-flow hive-mind task -d "Implement auth module"', description: 'Submit task' },
    { command: 'arcanea-flow hive-mind task -d "Security review" -p critical -c', description: 'Critical task with consensus' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      output.writeln(output.dim(`  Track with: arcanea-flow hive-mind task-status ${result.taskId}`));
...
export const hiveMindCommand: Command = {
  ...
  options: [],
  examples: [
    { command: 'arcanea-flow hive-mind init -t hierarchical-mesh', description: 'Initialize hive' },
    { command: 'arcanea-flow hive-mind spawn -n 5', description: 'Spawn workers' },
    { command: 'arcanea-flow hive-mind spawn --claude -o "Build a feature"', description: 'Launch Claude Code with hive mind' },
    { command: 'arcanea-flow hive-mind task -d "Build feature"', description: 'Submit task' }
  ],
  action: async (): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold('Hive Mind - Consensus-Based Multi-Agent Coordination'));
    output.writeln();
    output.writeln('Usage: arcanea-flow hive-mind <subcommand> [options]');
    output.writeln();
    ...
    output.writeln(output.dim('  arcanea-flow hive-mind init'));
    output.writeln(output.dim('  arcanea-flow hive-mind spawn -n 5 --claude -o "Your objective here"'));
