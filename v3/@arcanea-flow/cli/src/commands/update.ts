 ...
 * Auto-update system for @arcanea-flow packages (ADR-025)
 */
...
const checkCommand: Command = {
  name: 'check',
  description: 'Check for available @arcanea-flow package updates',
  options: [
  ...
  async action(ctx: CommandContext): Promise<CommandResult> {
    ...
    try {
      ...
      if (results.length === 0) {
        output.printSuccess('All @arcanea-flow packages are up to date!');
        return { success: true };
      }
      ...
      if (manualUpdates.length > 0) {
        ...
        output.writeln('  arcanea-flow update all');
      }
...
const allCommand: Command = {
  name: 'all',
  description: 'Update all @arcanea-flow packages',
  options: [
...
const updateCommand: Command = {
  name: 'update',
  description: 'Manage @arcanea-flow package updates (ADR-025)',
  ...
  async action(): Promise<CommandResult> {
    ...
    output.writeln();
    output.writeln('Manage @arcanea-flow package updates with auto-update support.');
    output.writeln();
    ...
    output.writeln();
    output.writeln('Run "arcanea-flow update <subcommand> --help" for subcommand help');
