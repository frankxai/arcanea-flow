...
const daemonCommand: Command = {
  ...
  options: [
    ...
    },
    {
      ...
      default: '.arcanea-flow/daemon.pid',
    },
    {
      name: 'log-file',
      type: 'string',
      description: 'Log file location',
      default: '.arcanea-flow/daemon.log',
    },
    {
      name: 'detach',
      type: 'boolean',
  ...
  examples: [
    { command: 'arcanea-flow process daemon --action start', description: 'Start the daemon' },
    { command: 'arcanea-flow process daemon --action stop', description: 'Stop the daemon' },
    { command: 'arcanea-flow process daemon --action restart --port 3850', description: 'Restart on different port' },
    { command: 'arcanea-flow process daemon --action status', description: 'Check daemon status' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const action = (ctx.flags?.action as string) || 'status';
    const port = (ctx.flags?.port as number) || 3847;
    const pidFile = (ctx.flags?.['pid-file'] as string) || '.arcanea-flow/daemon.pid';
    const logFile = (ctx.flags?.['log-file'] as string) || '.arcanea-flow/daemon.log';
    ...
    switch (action) {
      ...
      case 'restart':
        console.log('\n🔄 Restarting arcanea-flow daemon...\n');
        if (existingDaemon) {
          console.log(`  🛑 Stopping PID ${existingDaemon.pid}...`);
          removePidFile(pidFile);
          console.log('  ✅ Stopped');
      ...
      case 'status':
        console.log('\n📊 Daemon Status\n');
        console.log('  ┌─────────────────────────────────────────┐');
        console.log('  │ arcanea-flow daemon                      │');
        console.log('  ├─────────────────────────────────────────┤');
        if (existingDaemon) {
        ...
        console.log('  └─────────────────────────────────────────┘');
        if (!existingDaemon) {
          console.log('\n  To start: arcanea-flow process daemon --action start');
        }
        break;
    }
...
const monitorCommand: Command = {
  ...
  examples: [
    { command: 'arcanea-flow process monitor', description: 'Show process dashboard' },
    { command: 'arcanea-flow process monitor --watch --interval 5', description: 'Watch mode' },
    { command: 'arcanea-flow process monitor --components agents,memory,tasks', description: 'Monitor specific components' },
    { command: 'arcanea-flow process monitor --format json', description: 'JSON output' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const workersCommand: Command = {
  ...
  examples: [
    { command: 'arcanea-flow process workers --action list', description: 'List all workers' },
    { command: 'arcanea-flow process workers --action spawn --type task --count 3', description: 'Spawn task workers' },
    { command: 'arcanea-flow process workers --action kill --id worker-123', description: 'Kill specific worker' },
    { command: 'arcanea-flow process workers --action scale --type memory --count 5', description: 'Scale memory workers' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const signalsCommand: Command = {
  ...
  examples: [
    { command: 'arcanea-flow process signals --target daemon --signal graceful-shutdown', description: 'Graceful shutdown' },
    { command: 'arcanea-flow process signals --target workers --signal pause', description: 'Pause workers' },
    { command: 'arcanea-flow process signals --target all --signal reload-config', description: 'Reload all configs' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const logsCommand: Command = {
  ...
  examples: [
    { command: 'arcanea-flow process logs', description: 'Show recent logs' },
    { command: 'arcanea-flow process logs --source daemon --tail 100', description: 'Daemon logs' },
    { command: 'arcanea-flow process logs --follow --level error', description: 'Follow error logs' },
    { command: 'arcanea-flow process logs --since 1h --grep "error"', description: 'Search logs' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
export const processCommand: Command = {
  ...
  examples: [
    { command: 'arcanea-flow process daemon --action start', description: 'Start daemon' },
    { command: 'arcanea-flow process monitor --watch', description: 'Watch processes' },
    { command: 'arcanea-flow process workers --action list', description: 'List workers' },
    { command: 'arcanea-flow process logs --follow', description: 'Follow logs' },
  ],
  action: async (_ctx: CommandContext): Promise<CommandResult> => {
    ...
    console.log('\nExamples:');
    console.log('  arcanea-flow process daemon --action start');
    console.log('  arcanea-flow process monitor --watch');
    console.log('  arcanea-flow process workers --action spawn --type task --count 3');
    console.log('  arcanea-flow process logs --follow --level error');
