...
function isInitialized(cwd: string): { claude: boolean; claudeFlow: boolean } {
  ...
  const claudeFlowPath = path.join(cwd, '.arcanea-flow', 'config.yaml');
  return {
...
const initAction = async (ctx: CommandContext): Promise<CommandResult> => {
  ...
  if (hasExisting && !force) {
    ...
    if (initialized.claudeFlow) output.printInfo('  Found: .arcanea-flow/config.yaml');
  ...
  try {
    ...
    if (options.components.runtime) {
      output.printBox(
        [
          `Config:      .arcanea-flow/config.yaml`,
          `Data:        .arcanea-flow/data/`,
          `Logs:        .arcanea-flow/logs/`,
          `Sessions:    .arcanea-flow/sessions/`,
        ].join('\n'),
        'V3 Runtime'
      );
      output.writeln();
    ...
    if (startDaemon || startAll) {
      ...
      if (startAll) {
        try {
          ...
          execSync('npx @arcanea-flow/cli@latest memory init 2>/dev/null', {
            stdio: 'pipe',
            cwd: ctx.cwd,
            timeout: 30000
          });
      ...
      if (startDaemon) {
        try {
          ...
          execSync('npx @arcanea-flow/cli@latest daemon start 2>/dev/null &', {
            stdio: 'pipe',
            cwd: ctx.cwd,
            timeout: 10000
          });
      ...
      if (startAll) {
        try {
          ...
          execSync('npx @arcanea-flow/cli@latest swarm init --topology hierarchical 2>/dev/null', {
            stdio: 'pipe',
            cwd: ctx.cwd,
            timeout: 30000
          });
    ...
    if (withEmbeddings) {
      ...
      try {
        ...
        execSync(`npx @arcanea-flow/cli@latest embeddings init --model ${embeddingModel} --no-download --force 2>/dev/null`, {
          stdio: 'pipe',
          cwd: ctx.cwd,
          timeout: 30000
        });
      ...
      if (preset === 'minimal') {
        ...
        const components = await multiSelect({
          ...
          options: [
            ...
            { value: 'runtime', label: 'Runtime', hint: '.arcanea-flow/ directory for V3 runtime', selected: true },
          ],
        });
        ...
        if (options.components.settings) {
          const hooks = await multiSelect({
            ...
            options: [
              ...
              { value: 'permissionRequest', label: 'PermissionRequest', hint: 'Auto-allow arcanea-flow tools', selected: true },
            ],
          });
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    const result = {
      ...
      paths: {
        ...
        claudeFlowConfig: initialized.claudeFlow ? path.join(ctx.cwd, '.arcanea-flow', 'config.yaml') : null,
      },
    };
    ...
    if (result.initialized) {
      ...
      if (initialized.claudeFlow) {
        output.printInfo(`  V3 Runtime: .arcanea-flow/config.yaml`);
      }
    } else {
