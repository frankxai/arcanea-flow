/**
 * V3 CLI Init Command
 * Comprehensive initialization for Arcanea Flow with Claude Code integration
 */
...
function getLocalCliCommand(cwd: string): string {
  const launcher = process.platform === 'win32'
    ? path.join(cwd, 'node_modules', '.bin', 'arcanea-flow.cmd')
    : path.join(cwd, 'node_modules', '.bin', 'arcanea-flow');
...
const initAction = async (ctx: CommandContext): Promise<CommandResult> => {
  ...
  if (hasExisting && !force) {
    output.printWarning('Arcanea Flow appears to be already initialized');
  ...
  output.writeln();
  output.writeln(output.bold('Initializing Arcanea Flow V3'));
  output.writeln();
  ...
  try {
    ...
    if (!result.success) {
      ...
      return { success: false, exitCode: 1 };
        ...
        spinner.succeed('Arcanea Flow V3 initialized successfully!');
    output.writeln();
    ...
    if (startDaemon || startAll) {
      ...
      if (startAll) {
        try {
          ...
          execSync(`${getLocalCliCommand(ctx.cwd)} memory init`, {
            stdio: 'pipe',
            cwd: ctx.cwd,
            timeout: 30000
          });
      ...
      if (startDaemon) {
        try {
          ...
          execSync(`${getLocalCliCommand(ctx.cwd)} daemon start`, {
            stdio: 'pipe',
            cwd: ctx.cwd,
            timeout: 10000
          });
      ...
      if (startAll) {
        try {
          ...
          execSync(`${getLocalCliCommand(ctx.cwd)} swarm init --topology hierarchical`, {
            stdio: 'pipe',
            cwd: ctx.cwd,
            timeout: 30000
          });
    ...
    if (withEmbeddings) {
      ...
      try {
        ...
        execSync(`${getLocalCliCommand(ctx.cwd)} embeddings init --model ${embeddingModel} --no-download --force`, {
          stdio: 'pipe',
          cwd: ctx.cwd,
          timeout: 30000
        });
    ...
    if (!startDaemon && !startAll) {
      ...
      output.printList([
                `Run ${output.highlight('arcanea-flow daemon start')} to start background workers`,
                `Run ${output.highlight('arcanea-flow memory init')} to initialize memory database`,
                `Run ${output.highlight('arcanea-flow swarm init')} to initialize a swarm`,
                `Or use ${output.highlight('arcanea-flow init --start-all')} to do all of the above`,
...
const wizardCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    output.writeln();
        output.writeln(output.bold('Arcanea Flow V3 Setup Wizard'));
...
const checkCommand: Command = {
  name: 'check',
    description: 'Check if Arcanea Flow is initialized',
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (result.initialized) {
            output.printSuccess('Arcanea Flow is initialized');
      ...
      if (initialized.claudeFlow) {
        output.printInfo(`  V3 Runtime: .claude-flow/config.yaml`);
            ...
            output.printWarning('Arcanea Flow is not initialized in this directory');
            output.printInfo('Run "arcanea-flow init" to initialize');
    }
...
const upgradeCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.writeln();
        output.writeln(output.bold('Upgrading Arcanea Flow'));
    if (addMissing) {
...
export const initCommand: Command = {
  name: 'init',
  description: 'Initialize Arcanea Flow in the current directory',
  ...
  examples: [
        { command: 'arcanea-flow init', description: 'Initialize with default configuration' },
        { command: 'arcanea-flow init --start-all', description: 'Initialize and start daemon, memory, swarm' },
        { command: 'arcanea-flow init --start-daemon', description: 'Initialize and start daemon only' },
        { command: 'arcanea-flow init --minimal', description: 'Initialize with minimal configuration' },
        { command: 'arcanea-flow init --full', description: 'Initialize with all components' },
        { command: 'arcanea-flow init --force', description: 'Reinitialize and overwrite existing config' },
        { command: 'arcanea-flow init --only-claude', description: 'Only create Claude Code integration' },
        { command: 'arcanea-flow init --skip-claude', description: 'Only create V3 runtime' },
        { command: 'arcanea-flow init wizard', description: 'Interactive setup wizard' },
        { command: 'arcanea-flow init --with-embeddings', description: 'Initialize with ONNX embeddings' },
        { command: 'arcanea-flow init --with-embeddings --embedding-model all-mpnet-base-v2', description: 'Use larger embedding model' },
        { command: 'arcanea-flow init skills --all', description: 'Install all available skills' },
        { command: 'arcanea-flow init hooks --minimal', description: 'Create minimal hooks configuration' },
        { command: 'arcanea-flow init upgrade', description: 'Update helpers while preserving data' },
        { command: 'arcanea-flow init upgrade --verbose', description: 'Show detailed upgrade info' },
  ],
  action: initAction,
};
