...
const startCommand: Command = {
  ...
  examples: [
    { command: 'arcanea-flow daemon start', description: 'Start daemon in background (default)' },
    { command: 'arcanea-flow daemon start --foreground', description: 'Start in foreground (blocks terminal)' },
    { command: 'arcanea-flow daemon start -w map,audit,optimize', description: 'Start with specific workers' },
    { command: 'arcanea-flow daemon start --headless --sandbox strict', description: 'Start with headless workers in strict sandbox' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      const stateDir = join(projectRoot, '.arcanea-flow');
...
function validatePath(path: string, label: string): void {
  ...
  if (!resolved.includes('.arcanea-flow') && !resolved.includes('bin')) {
...
async function startBackgroundDaemon(projectRoot: string, quiet: boolean): Promise<CommandResult> {
  ...
  const stateDir = join(resolvedRoot, '.arcanea-flow');
  const pidFile = join(stateDir, 'daemon.pid');
  ...
  if (!quiet) {
    ...
    output.printInfo(`Stop with: arcanea-flow daemon stop`);
  }
...
const stopCommand: Command = {
  ...
  examples: [
    { command: 'arcanea-flow daemon stop', description: 'Stop the daemon' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const quiet = ctx.flags.quiet as boolean;
    const projectRoot = process.cwd();
...
async function killBackgroundDaemon(projectRoot: string): Promise<boolean> {
  const pidFile = join(projectRoot, '.arcanea-flow', 'daemon.pid');
...
const statusCommand: Command = {
  ...
  examples: [
    { command: 'arcanea-flow daemon status', description: 'Show daemon status' },
    { command: 'arcanea-flow daemon status -v', description: 'Show detailed status' },
    { command: 'arcanea-flow daemon status --show-modes', description: 'Show worker execution modes' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      output.printBox(
        [
          ...
          '',
          'Run "arcanea-flow daemon start" to start the daemon',
        ].join('\n'),
        'Worker Daemon'
      );
...
const triggerCommand: Command = {
  ...
  examples: [
    { command: 'arcanea-flow daemon trigger -w map', description: 'Trigger the map worker' },
    { command: 'arcanea-flow daemon trigger -w audit', description: 'Trigger security audit' },
    { command: 'arcanea-flow daemon trigger -w audit --headless', description: 'Trigger audit in headless sandbox' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const enableCommand: Command = {
  ...
  examples: [
    { command: 'arcanea-flow daemon enable -w predict', description: 'Enable predict worker' },
    { command: 'arcanea-flow daemon enable -w document --disable', description: 'Disable document worker' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const workerType = ctx.flags.worker as WorkerType;
...
export const daemonCommand: Command = {
  ...
  options: [],
  examples: [
    { command: 'arcanea-flow daemon start', description: 'Start the daemon' },
    { command: 'arcanea-flow daemon start --headless', description: 'Start with headless workers (E2B sandbox)' },
    { command: 'arcanea-flow daemon status', description: 'Check daemon status' },
    { command: 'arcanea-flow daemon stop', description: 'Stop the daemon' },
    { command: 'arcanea-flow daemon trigger -w audit', description: 'Run security audit' },
  ],
  action: async (): Promise<CommandResult> => {
    ...
    output.writeln();
    output.writeln('Run "arcanea-flow daemon <subcommand> --help" for details');
