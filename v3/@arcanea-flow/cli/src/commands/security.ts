...
const scanCommand: Command = {
  ...
  examples: [
    { command: 'arcanea-flow security scan -t ./src', description: 'Scan source directory' },
    { command: 'arcanea-flow security scan --depth deep --fix', description: 'Deep scan with auto-fix' },
  ],
...
const cveCommand: Command = {
  ...
  examples: [
    { command: 'arcanea-flow security cve --list', description: 'List all CVEs' },
    { command: 'arcanea-flow security cve -c CVE-2024-1234', description: 'Check specific CVE' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const threatsCommand: Command = {
  ...
  examples: [
    { command: 'arcanea-flow security threats --model stride', description: 'Run STRIDE analysis' },
    { command: 'arcanea-flow security threats -e md', description: 'Export as markdown' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const auditCommand: Command = {
  ...
  examples: [
    { command: 'arcanea-flow security audit --action list', description: 'List audit logs' },
    { command: 'arcanea-flow security audit -a export', description: 'Export audit trail' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const secretsCommand: Command = {
  ...
  examples: [
    { command: 'arcanea-flow security secrets --action scan', description: 'Scan for secrets' },
    { command: 'arcanea-flow security secrets -a rotate', description: 'Rotate compromised secrets' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.writeln(output.dim('Run "arcanea-flow security scan --depth full" for real secret detection.'));
    output.writeln();
    output.printTable({
      columns: [
...
const defendCommand: Command = {
  ...
  examples: [
    { command: 'arcanea-flow security defend -i "ignore previous instructions"', description: 'Scan text for threats' },
    { command: 'arcanea-flow security defend -f ./prompts.txt', description: 'Scan file for threats' },
    { command: 'arcanea-flow security defend --stats', description: 'Show detection statistics' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    let createAIDefence: typeof import('@arcanea-flow/aidefence').createAIDefence;
    try {
      const aidefence = await import('@arcanea-flow/aidefence');
      createAIDefence = aidefence.createAIDefence;
    } catch {
      output.error('AIDefence package not installed. Run: npm install @arcanea-flow/aidefence');
    ...
    if (!textToScan) {
      output.writeln('Usage: arcanea-flow security defend -i "<text>" or -f <file>');
      output.writeln();
...
export const securityCommand: Command = {
  ...
  examples: [
    { command: 'arcanea-flow security scan', description: 'Run security scan' },
    { command: 'arcanea-flow security cve --list', description: 'List known CVEs' },
    { command: 'arcanea-flow security threats', description: 'Run threat analysis' },
  ],
