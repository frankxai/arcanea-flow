...
const listCommand: Command = {
  ...
  examples: [
    { command: 'arcanea-flow providers list', description: 'List all providers' },
    { command: 'arcanea-flow providers list -t embedding', description: 'List embedding providers' },
  ],
...
const configureCommand: Command = {
  ...
  examples: [
    { command: 'arcanea-flow providers configure -p openai -k sk-...', description: 'Set OpenAI key' },
    { command: 'arcanea-flow providers configure -p anthropic -m claude-3.5-sonnet', description: 'Set default model' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const testCommand: Command = {
  ...
  examples: [
    { command: 'arcanea-flow providers test -p openai', description: 'Test OpenAI connection' },
    { command: 'arcanea-flow providers test --all', description: 'Test all providers' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const provider = ctx.flags.provider as string;
...
const modelsCommand: Command = {
  ...
  examples: [
    { command: 'arcanea-flow providers models', description: 'List all models' },
    { command: 'arcanea-flow providers models -p anthropic', description: 'List Anthropic models' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    output.writeln();
...
const usageCommand: Command = {
  ...
  examples: [
    { command: 'arcanea-flow providers usage', description: 'View all usage' },
    { command: 'arcanea-flow providers usage -t 30d', description: 'View 30-day usage' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
export const providersCommand: Command = {
  ...
  examples: [
    { command: 'arcanea-flow providers list', description: 'List all providers' },
    { command: 'arcanea-flow providers configure -p openai', description: 'Configure OpenAI' },
    { command: 'arcanea-flow providers test --all', description: 'Test all providers' },
  ],
