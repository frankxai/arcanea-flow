...
const trainCommand: Command = {
  ...
  examples: [
    { command: 'arcanea-flow neural train -p coordination -e 100', description: 'Train coordination patterns' },
    { command: 'arcanea-flow neural train -d ./training-data.json', description: 'Train from file' },
  ],
...
const statusCommand: Command = {
  ...
  examples: [
    { command: 'arcanea-flow neural status', description: 'Show all neural status' },
    { command: 'arcanea-flow neural status -m model-123', description: 'Check specific model' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const patternsCommand: Command = {
  ...
  examples: [
    { command: 'arcanea-flow neural patterns --action list', description: 'List all patterns' },
    { command: 'arcanea-flow neural patterns -a analyze -q "error handling"', description: 'Analyze patterns' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      output.writeln(output.dim('Run: arcanea-flow neural train --pattern-type general'));
...
const predictCommand: Command = {
  ...
  examples: [
    { command: 'arcanea-flow neural predict -m coord-v1 -i "route task to agent"', description: 'Make prediction' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const optimizeCommand: Command = {
  ...
  examples: [
    { command: 'arcanea-flow neural optimize -m model-v1 --method quantize', description: 'Quantize model' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const modelId = ctx.flags.model as string;
...
export const neuralCommand: Command = {
  ...
  examples: [
    { command: 'arcanea-flow neural status', description: 'Check neural system status' },
    { command: 'arcanea-flow neural train -p coordination', description: 'Train coordination patterns' },
    { command: 'arcanea-flow neural patterns --action list', description: 'List learned patterns' },
  ],
