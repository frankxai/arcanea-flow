...
function generateBashCompletion(): string {
  return `# arcanea-flow bash completion
# Generated by arcanea-flow completions bash
...
complete -F _claude_flow_completions arcanea-flow
complete -F _claude_flow_completions npx\\ @arcanea-flow/cli@v3alpha
`;
}

// Generate zsh completion script
function generateZshCompletion(): string {
  return `#compdef arcanea-flow
# arcanea-flow zsh completion
# Generated by arcanea-flow completions zsh
# Add to ~/.zfunc/_arcanea-flow or ~/.zsh/completions/_arcanea-flow
...
_claude_flow() {
    ...
    case $state in
        command)
            _describe -t commands 'arcanea-flow commands' commands
            ;;
        subcommand)
            case $words[2] in
                swarm)
...
function generateFishCompletion(): string {
  return `# arcanea-flow fish completion
# Generated by arcanea-flow completions fish
# Save to ~/.config/fish/completions/arcanea-flow.fish

# Disable file completion by default
complete -c arcanea-flow -f

# Top-level commands
${TOP_LEVEL_COMMANDS.map(cmd => `complete -c arcanea-flow -n "__fish_use_subcommand" -a "${cmd}"`).join('\n')}

# Swarm subcommands
${SWARM_SUBCOMMANDS.map(sub => `complete -c arcanea-flow -n "__fish_seen_subcommand_from swarm" -a "${sub}"`).join('\n')}

# Agent subcommands
${AGENT_SUBCOMMANDS.map(sub => `complete -c arcanea-flow -n "__fish_seen_subcommand_from agent" -a "${sub}"`).join('\n')}

# Task subcommands
${TASK_SUBCOMMANDS.map(sub => `complete -c arcanea-flow -n "__fish_seen_subcommand_from task" -a "${sub}"`).join('\n')}

# Memory subcommands
${MEMORY_SUBCOMMANDS.map(sub => `complete -c arcanea-flow -n "__fish_seen_subcommand_from memory" -a "${sub}"`).join('\n')}

# Hive-mind subcommands
${HIVE_MIND_SUBCOMMANDS.map(sub => `complete -c arcanea-flow -n "__fish_seen_subcommand_from hive-mind hive" -a "${sub}"`).join('\n')}

# Hooks subcommands
${HOOKS_SUBCOMMANDS.map(sub => `complete -c arcanea-flow -n "__fish_seen_subcommand_from hooks" -a "${sub}"`).join('\n')}

# Neural subcommands
complete -c arcanea-flow -n "__fish_seen_subcommand_from neural" -a "train status patterns predict optimize"

# Security subcommands
complete -c arcanea-flow -n "__fish_seen_subcommand_from security" -a "scan cve threats audit secrets"

# Performance subcommands
complete -c arcanea-flow -n "__fish_seen_subcommand_from performance" -a "benchmark profile metrics optimize bottleneck"

# Plugins subcommands
complete -c arcanea-flow -n "__fish_seen_subcommand_from plugins" -a "list install uninstall toggle info create"

# Deployment subcommands
complete -c arcanea-flow -n "__fish_seen_subcommand_from deployment deploy" -a "deploy status rollback history environments logs"

# Claims subcommands
complete -c arcanea-flow -n "__fish_seen_subcommand_from claims" -a "list check grant revoke roles policies"

# Embeddings subcommands
complete -c arcanea-flow -n "__fish_seen_subcommand_from embeddings" -a "generate search compare collections index providers"

# Daemon subcommands
complete -c arcanea-flow -n "__fish_seen_subcommand_from daemon" -a "start stop status trigger enable"

# Providers subcommands
complete -c arcanea-flow -n "__fish_seen_subcommand_from providers" -a "list configure test models usage"
`;
}

// Generate PowerShell completion script
function generatePowerShellCompletion(): string {
  return `# arcanea-flow PowerShell completion
# Generated by arcanea-flow completions powershell
...
Register-ArgumentCompleter -Native -CommandName arcanea-flow -ScriptBlock {
...
export const completionsCommand: Command = {
  ...
  options: [],
  examples: [
    { command: 'arcanea-flow completions bash > ~/.bash_completion.d/arcanea-flow', description: 'Install bash completions' },
    { command: 'arcanea-flow completions zsh > ~/.zfunc/_arcanea-flow', description: 'Install zsh completions' },
    { command: 'arcanea-flow completions fish > ~/.config/fish/completions/arcanea-flow.fish', description: 'Install fish completions' },
    { command: 'arcanea-flow completions powershell >> $PROFILE', description: 'Install PowerShell completions' }
  ],
  action: async (): Promise<CommandResult> => {
    ...
    output.writeln(output.dim('  arcanea-flow completions bash > ~/.bash_completion.d/arcanea-flow'));
    output.writeln(output.dim('  source ~/.bash_completion.d/arcanea-flow'));
    output.writeln();
    output.writeln(output.bold('Zsh:'));
    output.writeln(output.dim('  mkdir -p ~/.zfunc'));
    output.writeln(output.dim('  arcanea-flow completions zsh > ~/.zfunc/_arcanea-flow'));
    output.writeln(output.dim('  # Add to ~/.zshrc: fpath=(~/.zfunc $fpath); autoload -Uz compinit && compinit'));
    output.writeln();
    output.writeln(output.bold('Fish:'));
    output.writeln(output.dim('  arcanea-flow completions fish > ~/.config/fish/completions/arcanea-flow.fish'));
    output.writeln();
    output.writeln(output.bold('PowerShell:'));
    output.writeln(output.dim('  arcanea-flow completions powershell >> $PROFILE'));
