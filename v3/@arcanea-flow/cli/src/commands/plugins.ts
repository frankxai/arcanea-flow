...
const listCommand: Command = {
  ...
  options: [
    ...
    { name: 'registry', short: 'r', type: 'string', description: 'Registry to use (default: arcanea-flow-official)' },
  ],
  examples: [
    { command: 'arcanea-flow plugins list', description: 'List all plugins from registry' },
    { command: 'arcanea-flow plugins list --installed', description: 'List installed only' },
    { command: 'arcanea-flow plugins list --official', description: 'List official plugins' },
    { command: 'arcanea-flow plugins list --category security', description: 'List security plugins' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (installedOnly) {
      ...
      output.printTable({
        ...
        data: [
          { name: '@arcanea-flow/neural', version: '3.0.0', type: 'core', status: output.success('Active') },
          { name: '@arcanea-flow/security', version: '3.0.0', type: 'command', status: output.success('Active') },
          { name: '@arcanea-flow/embeddings', version: '3.0.0', type: 'core', status: output.success('Active') },
        ],
      });
...
const installCommand: Command = {
  ...
  examples: [
    { command: 'arcanea-flow plugins install -n community-analytics', description: 'Install plugin from IPFS' },
    { command: 'arcanea-flow plugins install -n ./my-plugin --dev', description: 'Install local plugin' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      if (!isLocalPath) {
        ...
        if (!plugin) {
          ...
          output.writeln();
          output.writeln(output.dim('Run "arcanea-flow plugins list" to see available plugins'));
          return { success: false, exitCode: 1 };
        }
...
const uninstallCommand: Command = {
  ...
  examples: [
    { command: 'arcanea-flow plugins uninstall -n community-analytics', description: 'Uninstall plugin' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const name = ctx.flags.name as string;
...
const toggleCommand: Command = {
  ...
  examples: [
    { command: 'arcanea-flow plugins toggle -n analytics --enable', description: 'Enable plugin' },
    { command: 'arcanea-flow plugins toggle -n analytics --disable', description: 'Disable plugin' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const name = ctx.flags.name as string;
...
const infoCommand: Command = {
  ...
  examples: [
    { command: 'arcanea-flow plugins info -n @arcanea-flow/neural', description: 'Show plugin info' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const name = ctx.flags.name as string;
    const registryName = ctx.flags.registry as string;
...
const createCommand: Command = {
  ...
  examples: [
    { command: 'arcanea-flow plugins create -n my-plugin', description: 'Create basic plugin' },
    { command: 'arcanea-flow plugins create -n my-plugin -t hooks', description: 'Create hooks plugin' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const name = ctx.flags.name as string;
...
const searchCommand: Command = {
  ...
  examples: [
    { command: 'arcanea-flow plugins search -q neural', description: 'Search for neural plugins' },
    { command: 'arcanea-flow plugins search -q security --verified', description: 'Search verified security plugins' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
export const pluginsCommand: Command = {
  ...
  examples: [
    { command: 'arcanea-flow plugins list', description: 'List plugins from IPFS registry' },
    { command: 'arcanea-flow plugins search -q neural', description: 'Search for plugins' },
    { command: 'arcanea-flow plugins install -n community-analytics', description: 'Install from IPFS' },
    { command: 'arcanea-flow plugins create -n my-plugin', description: 'Create new plugin' },
  ],
  action: async (): Promise<CommandResult> => {
    ...
    output.printList([
      '@arcanea-flow/neural     - Neural patterns and inference (WASM SIMD)',
      '@arcanea-flow/security   - Security scanning and CVE detection',
      '@arcanea-flow/embeddings - Vector embeddings with hyperbolic support',
      '@arcanea-flow/claims     - Claims-based authorization',
      '@arcanea-flow/performance- Performance profiling and benchmarks',
    ]);
    output.writeln();
    output.writeln(output.dim('Run "arcanea-flow plugins list --official" to see all official plugins'));
