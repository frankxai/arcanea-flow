...
export const ruvectorCommand: Command = {
  ...
  examples: [
    { command: 'arcanea-flow ruvector setup', description: 'Output Docker files and SQL for setup' },
    { command: 'arcanea-flow ruvector import --input memory.json', description: 'Import from sql.js/JSON export' },
    { command: 'arcanea-flow ruvector init --database mydb', description: 'Initialize RuVector in PostgreSQL' },
    { command: 'arcanea-flow ruvector status --verbose', description: 'Check connection and schema status' },
    { command: 'arcanea-flow ruvector migrate --up', description: 'Run pending migrations' },
    { command: 'arcanea-flow ruvector benchmark --vectors 10000', description: 'Run performance benchmark' },
    { command: 'arcanea-flow ruvector optimize --analyze', description: 'Analyze and suggest optimizations' },
    { command: 'arcanea-flow ruvector backup --output backup.sql', description: 'Backup RuVector data' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.writeln();
    output.printInfo('Run `arcanea-flow ruvector <command> --help` for details');
    output.writeln();
