...
export const initCommand: Command = {
  ...
  examples: [
    { command: 'arcanea-flow ruvector init -d mydb', description: 'Initialize with database name' },
    { command: 'arcanea-flow ruvector init -d mydb -h db.example.com --ssl', description: 'Remote with SSL' },
    { command: 'arcanea-flow ruvector init -d mydb --force', description: 'Force re-initialization' },
    { command: 'arcanea-flow ruvector init -d mydb --dimensions 768', description: 'Custom vector dimensions' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      output.printBox([
        ...
        '',
        'Next steps:',
        '  1. Run migrations: arcanea-flow ruvector migrate --up',
        '  2. Check status: arcanea-flow ruvector status --verbose',
        '  3. Run benchmark: arcanea-flow ruvector benchmark',
