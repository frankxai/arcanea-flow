...
export const optimizeCommand: Command = {
  ...
  examples: [
    { command: 'arcanea-flow ruvector optimize --analyze', description: 'Analyze and show recommendations' },
    { command: 'arcanea-flow ruvector optimize --apply', description: 'Apply optimizations' },
    { command: 'arcanea-flow ruvector optimize --vacuum', description: 'Run VACUUM ANALYZE' },
    { command: 'arcanea-flow ruvector optimize --reindex', description: 'Rebuild all indexes' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      output.printBox([
        ...
        '',
        'Quick commands:',
        `  arcanea-flow ruvector optimize --vacuum    # Clean up tables`,
        `  arcanea-flow ruvector optimize --reindex  # Rebuild indexes`,
        `  arcanea-flow ruvector optimize --apply    # Apply critical fixes`,
