 ...
 *   npx arcanea-flow ruvector import --from-memory
 *   npx arcanea-flow ruvector import --input data.json --batch-size 100
 *
...
export const importCommand: Command = {
  ...
  examples: [
    { command: 'arcanea-flow ruvector import --input memory-export.json', description: 'Import from JSON file' },
    { command: 'arcanea-flow ruvector import --input data.json --output import.sql', description: 'Generate SQL file (dry-run)' },
    { command: 'arcanea-flow ruvector import --from-memory', description: 'Export current memory and import' },
    { command: 'arcanea-flow ruvector import --input data.json --container my-postgres', description: 'Import using custom container' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (!inputFile && !fromMemory) {
      ...
      output.writeln('  arcanea-flow ruvector import --input memory-export.json');
      output.writeln('  arcanea-flow ruvector import --from-memory');
    ...
    if (fromMemory) {
      ...
      output.printWarning('Note: Run "npx arcanea-flow memory list --format json > memory-export.json" first');
      output.printInfo('Then use: npx arcanea-flow ruvector import --input memory-export.json');
