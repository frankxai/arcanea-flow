...
const statusCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const status = {
      ...
      components: [
        { name: 'Configuration', status: 'migrated', v2Path: './arcanea-flow.json', v3Path: './arcanea-flow.config.json' },
        { name: 'Memory Data', status: 'pending', v2Path: './.arcanea-flow/memory', v3Path: './data/memory' },
        { name: 'Agent Configs', status: 'pending', v2Path: './.arcanea-flow/agents', v3Path: './v3/agents' },
...
const runCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (backup && !dryRun) {
      ...
      output.writeln(output.dim(`  Backup created: ./.arcanea-flow-backup-${Date.now()}`));
      output.writeln();
    }
...
const breakingCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const changes = [
      {
        ...
        changes: [
          { change: 'Config file renamed', from: 'arcanea-flow.json', to: 'arcanea-flow.config.json' },
      ...
      },
      {
        ...
        changes: [
          ...
          { change: 'Storage path changed', from: '.arcanea-flow/memory', to: 'data/memory' }
        ]
      },
      {
        category: 'CLI',
    ...
    output.printInfo('Run "arcanea-flow migrate run" to automatically handle these changes');
...
export const migrateCommand: Command = {
  ...
  options: [],
  examples: [
    { command: 'arcanea-flow migrate status', description: 'Check migration status' },
    { command: 'arcanea-flow migrate run --dry-run', description: 'Preview migration' },
    { command: 'arcanea-flow migrate run -t all', description: 'Run full migration' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold('V2 to V3 Migration Tools'));
    output.writeln();
    output.writeln('Usage: arcanea-flow migrate <subcommand> [options]');
    output.writeln();
...
function getMigrationSteps(target: string): Array<{ name: string; description: string; source: string; dest: string }> {
  const allSteps = [
    { name: 'Configuration Files', description: 'Migrate config schema to V3 format', source: './arcanea-flow.json', dest: './arcanea-flow.config.json' },
    { name: 'Memory Backend', description: 'Upgrade to hybrid backend with AgentDB', source: './.arcanea-flow/memory', dest: './data/memory' },
    { name: 'Agent Definitions', description: 'Convert agent configs to V3 format', source: './.arcanea-flow/agents', dest: './v3/agents' },
    { name: 'Hook Registry', description: 'Migrate hooks to V3 hook system', source: './src/hooks', dest: './v3/hooks' },
    { name: 'Workflow Definitions', description: 'Convert workflows to event-sourced format', source: './.arcanea-flow/workflows', dest: './data/workflows' },
    { name: 'Embeddings System', description: 'Migrate to ONNX with hyperbolic (Poincaré ball)', source: 'OpenAI/TF.js embeddings', dest: '.arcanea-flow/embeddings.json' }
  ];
