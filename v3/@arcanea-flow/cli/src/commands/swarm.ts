...
const startCommand: Command = {
  ...
  examples: [
    { command: 'arcanea-flow swarm start -o "Build REST API" -s development', description: 'Start development swarm' },
    { command: 'arcanea-flow swarm start -o "Analyze codebase" --parallel', description: 'Parallel analysis' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.writeln(output.dim(`  Monitor: arcanea-flow swarm status ${executionState.swarmId}`));
...
const statusCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (!status.hasActiveSwarm) {
      ...
      output.writeln(output.dim('  npx @arcanea-flow/cli@latest swarm init'));
      output.writeln(output.dim('  npx @arcanea-flow/cli@latest swarm start'));
      output.writeln();
      return { success: true, data: status };
    }
...
export const swarmCommand: Command = {
  ...
  options: [],
  examples: [
    { command: 'arcanea-flow swarm init --v3-mode', description: 'Initialize V3 swarm' },
    { command: 'arcanea-flow swarm start -o "Build API" -s development', description: 'Start development swarm' },
    { command: 'arcanea-flow swarm coordinate --agents 15', description: 'V3 coordination' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold('Swarm Coordination Commands'));
    output.writeln();
    output.writeln('Usage: arcanea-flow swarm <subcommand> [options]');
    output.writeln();
