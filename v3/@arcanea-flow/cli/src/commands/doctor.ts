...
async function checkConfigFile(): Promise<HealthCheck> {
  ...
  return { name: 'Config File', status: 'warn', message: 'No config file (using defaults)', fix: 'arcanea-flow config init' };
}

// Check daemon status
async function checkDaemonStatus(): Promise<HealthCheck> {
  try {
    const pidFile = '.arcanea-flow/daemon.pid';
    if (existsSync(pidFile)) {
      ...
      try {
        ...
        return { name: 'Daemon Status', status: 'warn', message: 'Stale PID file', fix: 'rm .arcanea-flow/daemon.pid && arcanea-flow daemon start' };
      }
    }
    return { name: 'Daemon Status', status: 'warn', message: 'Not running', fix: 'arcanea-flow daemon start' };
  } catch {
    return { name: 'Daemon Status', status: 'warn', message: 'Unable to check', fix: 'arcanea-flow daemon status' };
  }
}
...
async function checkMemoryDatabase(): Promise<HealthCheck> {
  ...
  return { name: 'Memory Database', status: 'warn', message: 'Not initialized', fix: 'arcanea-flow memory configure --backend hybrid' };
}
...
async function checkMcpServers(): Promise<HealthCheck> {
  ...
  for (const configPath of mcpConfigPaths) {
    if (existsSync(configPath)) {
      try {
        ...
        const hasClaudeFlow = 'arcanea-flow' in servers || 'arcanea-flow_alpha' in servers;
        if (hasClaudeFlow) {
          return { name: 'MCP Servers', status: 'pass', message: `${count} servers (arcanea-flow configured)` };
        } else {
          return { name: 'MCP Servers', status: 'warn', message: `${count} servers (arcanea-flow not found)`, fix: 'claude mcp add arcanea-flow npx @arcanea-flow/cli@v3alpha mcp start' };
        }
      } catch {
  ...
  return { name: 'MCP Servers', status: 'warn', message: 'No MCP config found', fix: 'claude mcp add arcanea-flow npx @arcanea-flow/cli@v3alpha mcp start' };
}
  ...
  try {
    ...
    try {
      ...
      const possiblePaths = [
        ...
        join(process.cwd(), 'node_modules/@arcanea-flow/cli/package.json')
      ];
        ...
        try {
          ...
          if (existsSync(npxCacheDir)) {
            ...
            for (const entry of cacheEntries) {
              const npxPkgPath = join(npxCacheDir, entry, 'node_modules', '@arcanea-flow', 'cli', 'package.json');
      ...
      for (const pkgPath of possiblePaths) {
        try {
          if (existsSync(pkgPath)) {
            ...
            if (packageJson.name === '@arcanea-flow/cli' && packageJson.version) {
    ...
    try {
      const npmInfo = await runCommand('npm view @arcanea-flow/cli@alpha version 2>/dev/null', 5000);
    ...
    if (isOutdated) {
      const fix = isNpx
        ? 'rm -rf ~/.npm/_npx/* && npx -y @arcanea-flow/cli@latest'
        : 'npm update @arcanea-flow/cli';
...
export const doctorCommand: Command = {
  ...
  examples: [
    { command: 'arcanea-flow doctor', description: 'Run full health check' },
    { command: 'arcanea-flow doctor --fix', description: 'Show fixes for issues' },
    { command: 'arcanea-flow doctor --install', description: 'Auto-install missing dependencies' },
    { command: 'arcanea-flow doctor -c version', description: 'Check for stale npx cache' },
    { command: 'arcanea-flow doctor -c claude', description: 'Check Claude Code CLI only' }
  ],
