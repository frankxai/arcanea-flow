...
const storeCommand: Command = {
  ...
  examples: [
    { command: 'arcanea-flow memory store -k "api/auth" -v "JWT implementation"', description: 'Store text' },
    { command: 'arcanea-flow memory store -k "pattern/singleton" --vector', description: 'Store vector' }
  ],
...
const searchCommand: Command = {
  ...
  examples: [
    { command: 'arcanea-flow memory search -q "authentication patterns"', description: 'Semantic search' },
    { command: 'arcanea-flow memory search -q "JWT" -t keyword', description: 'Keyword search' },
    { command: 'arcanea-flow memory search -q "test" --build-hnsw', description: 'Build HNSW index and search' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      if (results.length === 0) {
        ...
        output.writeln(output.dim('Try: arcanea-flow memory store -k "key" --value "data"'));
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      if (entries.length === 0) {
        ...
        output.printInfo('Store data: arcanea-flow memory store -k "key" --value "data"');
        return { success: true, data: [] };
      }
...
const deleteCommand: Command = {
  ...
  examples: [
    { command: 'arcanea-flow memory delete -k "mykey"', description: 'Delete entry with default namespace' },
    { command: 'arcanea-flow memory delete -k "lesson" -n "lessons"', description: 'Delete entry from specific namespace' },
    { command: 'arcanea-flow memory delete mykey -f', description: 'Delete without confirmation' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const cleanupCommand: Command = {
  ...
  examples: [
    { command: 'arcanea-flow memory cleanup --dry-run', description: 'Preview cleanup' },
    { command: 'arcanea-flow memory cleanup --older-than 30d', description: 'Delete entries older than 30 days' },
    { command: 'arcanea-flow memory cleanup --expired-only', description: 'Clean expired entries' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const compressCommand: Command = {
  ...
  examples: [
    { command: 'arcanea-flow memory compress', description: 'Balanced compression' },
    { command: 'arcanea-flow memory compress --quantize --bits 4', description: '4-bit quantization (32x reduction)' },
    { command: 'arcanea-flow memory compress -l max -t vectors', description: 'Max compression on vectors' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const exportCommand: Command = {
  ...
  examples: [
    { command: 'arcanea-flow memory export -o ./backup.json', description: 'Export all to JSON' },
    { command: 'arcanea-flow memory export -o ./data.csv -f csv', description: 'Export to CSV' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const importCommand: Command = {
  ...
  examples: [
    { command: 'arcanea-flow memory import -i ./backup.json', description: 'Import from file' },
    { command: 'arcanea-flow memory import -i ./data.json -n archive', description: 'Import to namespace' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const initMemoryCommand: Command = {
  ...
  examples: [
    { command: 'arcanea-flow memory init', description: 'Initialize hybrid backend with all features' },
    { command: 'arcanea-flow memory init -b agentdb', description: 'Initialize AgentDB backend' },
    { command: 'arcanea-flow memory init -p ./data/memory.db --force', description: 'Reinitialize at custom path' },
    { command: 'arcanea-flow memory init --verbose --verify', description: 'Initialize with full verification' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      output.printList([
        `Store data: ${output.highlight('arcanea-flow memory store -k "key" --value "data"')}`,
        `Search: ${output.highlight('arcanea-flow memory search -q "query"')}`,
        `Train patterns: ${output.highlight('arcanea-flow neural train -p coordination')}`,
        `View stats: ${output.highlight('arcanea-flow memory stats')}`
      ]);
...
export const memoryCommand: Command = {
  ...
  options: [],
  examples: [
    { command: 'arcanea-flow memory store -k "key" -v "value"', description: 'Store data' },
    { command: 'arcanea-flow memory search -q "auth patterns"', description: 'Search memory' },
    { command: 'arcanea-flow memory stats', description: 'Show statistics' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold('Memory Management Commands'));
    output.writeln();
    output.writeln('Usage: arcanea-flow memory <subcommand> [options]');
    output.writeln();
