...
export function generateMCPConfig(options: InitOptions): object {
  ...
  if (config.claudeFlow) {
    mcpServers['arcanea-flow'] = createMCPServerEntry(
      ['@arcanea-flow/cli@latest', 'mcp', 'start'],
      {
        CLAUDE_FLOW_MODE: 'v3',
...
export function generateMCPCommands(options: InitOptions): string[] {
  ...
  if (config.claudeFlow) {
    commands.push(`claude mcp add arcanea-flow -- ${prefix}npx @arcanea-flow/cli@latest mcp start`);
  }
