...
function getV3Progress() {
  ...
  const dddPath = path.join(process.cwd(), '.arcanea-flow', 'metrics', 'ddd-progress.json');
...
function getSecurityStatus() {
  ...
  const auditStatusPath = path.join(process.cwd(), '.arcanea-flow', 'security', 'audit-status.json');
...
function getSwarmStatus() {
  ...
  const activityPath = path.join(process.cwd(), '.arcanea-flow', 'metrics', 'swarm-activity.json');
  ...
  const progressPath = path.join(process.cwd(), '.arcanea-flow', 'metrics', 'v3-progress.json');
  ...
  try {
    if (isWindows) {
      ...
      try {
        const ps = execSync('ps aux 2>/dev/null | grep -E "(agentic-flow|arcanea-flow|mcp.*server)" | grep -v grep | wc -l', { encoding: 'utf-8' });
...
function getSystemMetrics() {
  ...
  const learningMetricsPath = path.join(process.cwd(), '.arcanea-flow', 'metrics', 'learning.json');
  ...
  if (intelligencePct === 0) {
    ...
    const sessionPaths = [
      ...
      path.join(process.cwd(), '.arcanea-flow', 'sessions'),
    ];
  ...
  if (subAgents === 0 && !isWindows) {
    try {
      const agents = execSync('ps aux 2>/dev/null | grep -c "arcanea-flow.*agent" || echo "0"', { encoding: 'utf-8' });
...
function getADRStatus() {
  ...
  const compliancePath = path.join(process.cwd(), '.arcanea-flow', 'metrics', 'adr-compliance.json');
  ...
  const adrPaths = [
    ...
    path.join(process.cwd(), 'ADR'),
    path.join(process.cwd(), '.arcanea-flow', 'adrs'),
...
function getAgentDBStats() {
  ...
  const hnswPaths = [
    ...
    path.join(process.cwd(), '.arcanea-flow', 'hnsw.index'),
...
function getIntegrationStatus() {
  ...
  // Check for cache
  const cachePaths = [
    path.join(process.cwd(), '.arcanea-flow', 'cache'),
...
function getSessionStats() {
  ...
  const sessionPaths = [
    path.join(process.cwd(), '.arcanea-flow', 'session.json'),
  ...
  if (!duration) {
    const metricsPath = path.join(process.cwd(), '.arcanea-flow', 'metrics', 'activity.json');
...
try {
  const trendPath = path.join(process.cwd(), '.arcanea-flow', '.trend-cache.json');
