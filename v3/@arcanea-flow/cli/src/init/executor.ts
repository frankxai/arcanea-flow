...
const COMMANDS_MAP: Record<string, string[]> = {
  core: ['arcanea-flow-help.md', 'arcanea-flow-swarm.md', 'arcanea-flow-memory.md'],
  analysis: ['analysis'],
...
const DIRECTORIES = {
  ...
  runtime: [
    ...
    '.arcanea-flow/data',
    '.arcanea-flow/logs',
    '.arcanea-flow/sessions',
    '.arcanea-flow/hooks',
    '.arcanea-flow/agents',
    '.arcanea-flow/workflows',
  ],
};
...
export async function executeUpgrade(targetDir: string): Promise<UpgradeResult> {
  ...
  try {
    ...
    const metricsDir = path.join(targetDir, '.arcanea-flow', 'metrics');
    const securityDir = path.join(targetDir, '.arcanea-flow', 'security');
    ...
    if (!fs.existsSync(progressPath)) {
      ...
      result.created.push('.arcanea-flow/metrics/v3-progress.json');
    } else {
      result.preserved.push('.arcanea-flow/metrics/v3-progress.json');
    }
    ...
    if (!fs.existsSync(activityPath)) {
      ...
      result.created.push('.arcanea-flow/metrics/swarm-activity.json');
    } else {
      result.preserved.push('.arcanea-flow/metrics/swarm-activity.json');
    }
    ...
    if (!fs.existsSync(learningPath)) {
      ...
      result.created.push('.arcanea-flow/metrics/learning.json');
    } else {
      result.preserved.push('.arcanea-flow/metrics/learning.json');
    }
    ...
    if (!fs.existsSync(auditPath)) {
      const audit = {
        ...
        totalCves: 3,
        lastScan: null,
        _note: 'Run: npx @arcanea-flow/cli@latest security scan'
      };
      fs.writeFileSync(auditPath, JSON.stringify(audit, null, 2), 'utf-8');
      result.created.push('.arcanea-flow/security/audit-status.json');
    } else {
      result.preserved.push('.arcanea-flow/security/audit-status.json');
    }
...
function findSourceHelpersDir(sourceBaseDir?: string): string | null {
  ...
  // __dirname is typically /path/to/node_modules/@arcanea-flow/cli/dist/src/init
...
/**
 * Write runtime configuration (.arcanea-flow/)
 */
async function writeRuntimeConfig(
  ...
  const configPath = path.join(targetDir, '.arcanea-flow', 'config.yaml');

  if (fs.existsSync(configPath) && !options.force) {
    result.skipped.push('.arcanea-flow/config.yaml');
    return;
  }
...
memory:
  ...
  persistPath: .arcanea-flow/data
  cacheSize: 100

neural:
  enabled: ${options.runtime.enableNeural}
  modelPath: .arcanea-flow/neural
...
`;
  ...
  result.created.files.push('.arcanea-flow/config.yaml');

  // Write .gitignore
  const gitignorePath = path.join(targetDir, '.arcanea-flow', '.gitignore');
...
`;
  ...
  if (!fs.existsSync(gitignorePath) || options.force) {
    ...
    result.created.files.push('.arcanea-flow/.gitignore');
  }
...
async function writeInitialMetrics(
  ...
  const metricsDir = path.join(targetDir, '.arcanea-flow', 'metrics');
  const learningDir = path.join(targetDir, '.arcanea-flow', 'learning');
  const securityDir = path.join(targetDir, '.arcanea-flow', 'security');
  ...
  if (!fs.existsSync(progressPath) || options.force) {
    const progress = {
      ...
      _note: 'Metrics will update as you use Claude Flow. Run: npx @arcanea-flow/cli@latest daemon start'
    };
    fs.writeFileSync(progressPath, JSON.stringify(progress, null, 2), 'utf-8');
    result.created.files.push('.arcanea-flow/metrics/v3-progress.json');
  }
  ...
  if (!fs.existsSync(activityPath) || options.force) {
    ...
    fs.writeFileSync(activityPath, JSON.stringify(activity, null, 2), 'utf-8');
    result.created.files.push('.arcanea-flow/metrics/swarm-activity.json');
  }
  ...
  if (!fs.existsSync(learningPath) || options.force) {
    ...
    fs.writeFileSync(learningPath, JSON.stringify(learning, null, 2), 'utf-8');
    result.created.files.push('.arcanea-flow/metrics/learning.json');
  }
  ...
  if (!fs.existsSync(auditPath) || options.force) {
    ...
    fs.writeFileSync(auditPath, JSON.stringify(audit, null, 2), 'utf-8');
    result.created.files.push('.arcanea-flow/security/audit-status.json');
  }
}
...
async function writeCapabilitiesDoc(
  ...
  const capabilitiesPath = path.join(targetDir, '.arcanea-flow', 'CAPABILITIES.md');

  if (fs.existsSync(capabilitiesPath) && !options.force) {
    result.skipped.push('.arcanea-flow/CAPABILITIES.md');
    return;
  }

  const capabilities = `# Claude Flow V3 - Complete Capabilities Reference
> Generated: ${new Date().toISOString()}
> Full documentation: https://github.com/ruvnet/arcanea-flow
...
# Check status
npx @arcanea-flow/cli@latest swarm status

# Monitor activity
npx @arcanea-flow/cli@latest swarm monitor
\`\`\`
...
# Memory operations
npx @arcanea-flow/cli@latest memory store --key "pattern" --value "data" --namespace patterns
npx @arcanea-flow/cli@latest memory search --query "authentication"
...
\`\`\`bash
# Store pattern
npx @arcanea-flow/cli@latest memory store --key "name" --value "data" --namespace patterns

# Semantic search
npx @arcanea-flow/cli@latest memory search --query "authentication"

# List entries
npx @arcanea-flow/cli@latest memory list --namespace patterns

# Initialize database
npx @arcanea-flow/cli@latest memory init --force
\`\`\`
...
\`\`\`bash
# Initialize
npx @arcanea-flow/cli@latest hive-mind init --queen-type strategic

# Status
npx @arcanea-flow/cli@latest hive-mind status

# Spawn workers
npx @arcanea-flow/cli@latest hive-mind spawn --count 5 --type worker

# Consensus
npx @arcanea-flow/cli@latest hive-mind consensus --propose "task"
\`\`\`
...
# Swarm
npx @arcanea-flow/cli@latest swarm init --topology hierarchical --max-agents 8
npx @arcanea-flow/cli@latest swarm status

# Agents
npx @arcanea-flow/cli@latest agent spawn -t coder
npx @arcanea-flow/cli@latest agent list

# Memory
npx @arcanea-flow/cli@latest memory search --query "patterns"

# Hooks
npx @arcanea-flow/cli@latest hooks pre-task --description "task"
npx @arcanea-flow/cli@latest hooks worker dispatch --trigger optimize
\`\`\`
...
`;
  ...
  result.created.files.push('.arcanea-flow/CAPABILITIES.md');
}
