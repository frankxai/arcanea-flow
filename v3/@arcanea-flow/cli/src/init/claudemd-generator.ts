...
\`\`\`bash
npx @arcanea-flow/cli@latest hooks pre-task --description "[task description]"
\`\`\`
...
npx @arcanea-flow/cli@latest swarm init --topology hierarchical --max-agents 8 --strategy specialized

# Large teams (10-15 agents) - use hierarchical-mesh for V3 queen + peer communication
npx @arcanea-flow/cli@latest swarm init --topology hierarchical-mesh --max-agents 15 --strategy specialized
\`\`\`
...
Bash("npx @arcanea-flow/cli@latest swarm init --topology hierarchical --max-agents 8 --strategy specialized")
...
Bash("npx @arcanea-flow/cli@latest memory search --query '[task keywords]' --namespace patterns")

# 2. Check if similar task was done before
Bash("npx @arcanea-flow/cli@latest memory search --query '[task type]' --namespace tasks")

# 3. Load learned optimizations
Bash("npx @arcanea-flow/cli@latest hooks route --task '[task description]'")
\`\`\`

### After Completing Any Task Successfully
\`\`\`bash
# 1. Store successful pattern for future reference
Bash("npx @arcanea-flow/cli@latest memory store --namespace patterns --key '[pattern-name]' --value '[what worked]'")

# 2. Train neural patterns on the successful approach
Bash("npx @arcanea-flow/cli@latest hooks post-edit --file '[main-file]' --train-neural true")

# 3. Record task completion with metrics
Bash("npx @arcanea-flow/cli@latest hooks post-task --task-id '[id]' --success true --store-results true")

# 4. Trigger optimization worker if performance-related
Bash("npx @arcanea-flow/cli@latest hooks worker dispatch --trigger optimize")
\`\`\`
...
npx @arcanea-flow/cli@latest memory search --query "authentication patterns"
...
npx @arcanea-flow/cli@latest hooks post-edit --file "[file]" --train-neural true
...
npx @arcanea-flow/cli@latest hooks coverage-gaps --format table
npx @arcanea-flow/cli@latest hooks coverage-route --task "[task]"

# Statusline (for Claude Code integration)
npx @arcanea-flow/cli@latest hooks statusline
npx @arcanea-flow/cli@latest hooks statusline --json
\`\`\`
...
npx @arcanea-flow/cli@latest migrate status

# Run migration with backup
npx @arcanea-flow/cli@latest migrate run --backup

# Rollback if needed
npx @arcanea-flow/cli@latest migrate rollback

# Validate migration
npx @arcanea-flow/cli@latest migrate validate
\`\`\`
...
Bash("npx @arcanea-flow/cli@latest hooks post-command --command '[operation]' --track-metrics true")

# Periodically run benchmarks (every major feature)
Bash("npx @arcanea-flow/cli@latest performance benchmark --suite all")

# Analyze bottlenecks when performance degrades
Bash("npx @arcanea-flow/cli@latest performance profile --target '[component]'")
\`\`\`

### Session Persistence (Cross-Conversation Learning)
\`\`\`bash
# At session start - restore previous context
Bash("npx @arcanea-flow/cli@latest session restore --latest")

# At session end - persist learned patterns
Bash("npx @arcanea-flow/cli@latest hooks session-end --generate-summary true --persist-state true --export-metrics true")
\`\`\`

### Neural Pattern Training
\`\`\`bash
# Train on successful code patterns
Bash("npx @arcanea-flow/cli@latest neural train --pattern-type coordination --epochs 10")

# Predict optimal approach for new tasks
Bash("npx @arcanea-flow/cli@latest neural predict --input '[task description]'")

# View learned patterns
Bash("npx @arcanea-flow/cli@latest neural patterns --list")
\`\`\`
...
claude mcp add arcanea-flow -- npx -y @arcanea-flow/cli@latest
...
- **Memory store**: \`npx @arcanea-flow/cli@latest memory store --key "mykey" --value "myvalue" --namespace patterns\`
- **Memory search**: \`npx @arcanea-flow/cli@latest memory search --query "search terms"\`
- **Memory list**: \`npx @arcanea-flow/cli@latest memory list --namespace patterns\`
- **Memory retrieve**: \`npx @arcanea-flow/cli@latest memory retrieve --key "mykey" --namespace patterns\`
...
npx @arcanea-flow/cli@latest memory store --key "pattern-auth" --value "JWT with refresh tokens" --namespace patterns
npx @arcanea-flow/cli@latest memory store --key "bug-fix-123" --value "Fixed null check" --namespace solutions --tags "bugfix,auth"
\`\`\`
...
npx @arcanea-flow/cli@latest memory search --query "authentication patterns"
npx @arcanea-flow/cli@latest memory search --query "error handling" --namespace patterns --limit 5
\`\`\`

### List Entries
\`\`\`bash
# OPTIONAL: --namespace, --limit
npx @arcanea-flow/cli@latest memory list
npx @arcanea-flow/cli@latest memory list --namespace patterns --limit 10
\`\`\`
...
npx @arcanea-flow/cli@latest memory retrieve --key "pattern-auth"
npx @arcanea-flow/cli@latest memory retrieve --key "pattern-auth" --namespace patterns
\`\`\`

### Initialize Memory Database
\`\`\`bash
npx @arcanea-flow/cli@latest memory init --force --verbose
\`\`\`
...
**\`.arcanea-flow/CAPABILITIES.md\`** - Complete reference generated during init
...
# Core
npx @arcanea-flow/cli@latest init --wizard
npx @arcanea-flow/cli@latest agent spawn -t coder
npx @arcanea-flow/cli@latest swarm init --v3-mode
npx @arcanea-flow/cli@latest memory search --query "query"

# Advanced
npx @arcanea-flow/cli@latest daemon start
npx @arcanea-flow/cli@latest doctor --fix
npx @arcanea-flow/cli@latest security scan
npx @arcanea-flow/cli@latest performance benchmark
\`\`\`
...
See **\`.arcanea-flow/CAPABILITIES.md\`** for complete documentation of all 60+ agents, 26 commands, 27 hooks, and integrations.
