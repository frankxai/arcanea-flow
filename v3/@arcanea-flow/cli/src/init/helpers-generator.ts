...
for FILE in $STAGED_FILES; do
  if [[ "$FILE" =~ \\.(ts|js|tsx|jsx)$ ]]; then
    ...
    npx @arcanea-flow/cli hooks pre-edit --file "$FILE" --validate-syntax 2>/dev/null || true
  fi
done
...
# Notify arcanea-flow of commit
npx @arcanea-flow/cli hooks notify \\
...
const SESSION_DIR = path.join(process.cwd(), '.arcanea-flow', 'sessions');
...
const MEMORY_DIR = path.join(process.cwd(), '.arcanea-flow', 'data');
...
$ClaudeFlowDir = Join-Path $PWD '.arcanea-flow'
...
function Start-SwarmMonitor {
    ...
    $process = Start-Process -FilePath 'node' -ArgumentList @(
        '-e',
        'setInterval(() => { require("fs").writeFileSync(".arcanea-flow/metrics/swarm-activity.json", JSON.stringify({swarm:{active:true,agent_count:0},timestamp:Date.now()})) }, 5000)'
...
function getDataDir() {
  const localDir = path.join(process.cwd(), '.arcanea-flow', 'sessions');
  ...
  switch (platform) {
    case 'win32':
      return path.join(process.env.APPDATA || homeDir, 'arcanea-flow', 'sessions');
    case 'darwin':
      return path.join(homeDir, 'Library', 'Application Support', 'arcanea-flow', 'sessions');
    default:
      return path.join(homeDir, '.arcanea-flow', 'sessions');
  }
}
