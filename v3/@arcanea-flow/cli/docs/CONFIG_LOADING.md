...
1. **`src/config-adapter.ts`** (NEW)
   - Converts between `SystemConfig` (from `@arcanea-flow/shared`) and `V3Config` (CLI-specific format)
...
arcanea-flow agent spawn -t coder

# Use specific config file
arcanea-flow agent spawn -t coder --config ./custom-config.json

# Override with environment variables
CLAUDE_FLOW_MAX_AGENTS=20 arcanea-flow swarm init
```
...
3. **Validation** - Uses existing Zod schemas from `@arcanea-flow/shared`
...
- [ ] Config validation command (`arcanea-flow config validate`)
