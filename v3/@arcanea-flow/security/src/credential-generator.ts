...
export class CredentialGenerator {
  ...
  createEnvScript(credentials: GeneratedCredentials): string {
    return `# Arcanea Flow V3 - Generated Credentials
...
`;
  ...
  createJsonConfig(credentials: GeneratedCredentials): string {
    return JSON.stringify({
      'arcanea-flow/admin-password': credentials.adminPassword,
      'arcanea-flow/service-password': credentials.servicePassword,
      'arcanea-flow/jwt-secret': credentials.jwtSecret,
      'arcanea-flow/session-secret': credentials.sessionSecret,
      'arcanea-flow/encryption-key': credentials.encryptionKey,
      'arcanea-flow/generated-at': credentials.generatedAt.toISOString(),
      'arcanea-flow/expires-at': credentials.expiresAt?.toISOString() ?? null,
    }, null, 2);
  }
}
