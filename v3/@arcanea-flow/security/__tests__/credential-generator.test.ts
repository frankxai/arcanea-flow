...
describe('CredentialGenerator', () => {
  ...
  describe('JSON Config Generation', () => {
    ...
    it('should include all credentials', () => {
      ...
      expect(parsed['arcanea-flow/admin-password']).toBe(credentials.adminPassword);
      expect(parsed['arcanea-flow/service-password']).toBe(credentials.servicePassword);
      expect(parsed['arcanea-flow/jwt-secret']).toBe(credentials.jwtSecret);
      expect(parsed['arcanea-flow/session-secret']).toBe(credentials.sessionSecret);
      expect(parsed['arcanea-flow/encryption-key']).toBe(credentials.encryptionKey);
    });
  });
