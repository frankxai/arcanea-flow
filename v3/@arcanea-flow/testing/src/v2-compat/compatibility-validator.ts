...
export const V2_CLI_COMMANDS: V2CLICommand[] = [
  // Core commands
  { name: 'init', aliases: ['i'], flags: ['--force', '--template'], description: 'Initialize arcanea-flow project', v3Equivalent: 'init' },
...
export class V2CompatibilityValidator {
  ...
  async validateAPI(): Promise<ValidationResult> {
    ...
    for (const iface of V2_API_INTERFACES) {
      ...
      checks.push({
        ...
        v3Behavior: isSupported
          ? `Import "${v3Equivalent}" from @arcanea-flow/*`
          : 'Not available',
  ...
  private generateRecommendations(
    ...
    if (cli.breakingChanges > 0) {
      ...
      recommendations.push('Run migration script: npx @arcanea-flow/cli migrate');
    }
    ...
    if (api.breakingChanges > 0) {
      recommendations.push('Update import statements to use @arcanea-flow/* packages');
...
export function generateCompatibilityReport(report: FullValidationReport): string {
  ...
  lines.push("import { SwarmCoordinator } from 'arcanea-flow/swarm';");
  lines.push('');
  lines.push("// V3 imports (using aliases)");
  lines.push("import { UnifiedSwarmCoordinator as HiveMind } from '@arcanea-flow/swarm';");
  lines.push("import { UnifiedSwarmCoordinator as SwarmCoordinator } from '@arcanea-flow/swarm';");
  lines.push('```');
  lines.push('');
  lines.push('---');
  lines.push('');
