...
function generateEnhancedMarkdown(report: FullValidationReport, options: ReportOptions): string {
  ...
  lines.push('|-----------|-----------|');
  lines.push('| arcanea-flow/hive-mind | @arcanea-flow/swarm |');
  lines.push('| arcanea-flow/swarm | @arcanea-flow/swarm |');
  lines.push('| arcanea-flow/memory | @arcanea-flow/memory |');
  lines.push('| arcanea-flow/agents | @arcanea-flow/agent-lifecycle |');
  lines.push('| arcanea-flow/tasks | @arcanea-flow/task-execution |');
  lines.push('');
...
function generateExamplesSection(): string[] {
  ...
  lines.push('npx arcanea-flow hive-mind init');
  lines.push('npx arcanea-flow hive-mind status');
  lines.push('');
  lines.push('# V3 (recommended)');
  lines.push('npx @arcanea-flow/cli swarm init');
  lines.push('npx @arcanea-flow/cli swarm status');
  lines.push('```');
  lines.push('');
  ...
  lines.push("import { HiveMind } from 'arcanea-flow/hive-mind';");
  lines.push("import { MemoryManager } from 'arcanea-flow/memory';");
  lines.push('');
  lines.push('// V3 imports with aliases');
  lines.push("import { UnifiedSwarmCoordinator as HiveMind } from '@arcanea-flow/swarm';");
  lines.push("import { UnifiedMemoryService as MemoryManager } from '@arcanea-flow/memory';");
  lines.push('');
...
function generateMigrationScriptsSection(): string[] {
  ...
  lines.push('npx @arcanea-flow/cli migrate --from v2 --to v3');
  lines.push('');
  lines.push('# Migrate configuration');
  lines.push('npx @arcanea-flow/cli migrate config --input .arcanea-flow/config.yaml');
  lines.push('');
  lines.push('# Migrate memory database');
  lines.push('npx @arcanea-flow/cli migrate memory --input .arcanea-flow/memory.db');
  lines.push('```');
  lines.push('');
  ...
  lines.push('');
  lines.push('```yaml');
  lines.push('# V2 Configuration (.arcanea-flow/config.yaml)');
  lines.push('orchestrator:');
  lines.push('  maxAgents: 10');
  lines.push('  defaultStrategy: balanced');
  lines.push('memory:');
  lines.push('  backend: sqlite');
  lines.push('  path: ./.arcanea-flow/memory.db');
  lines.push('coordination:');
  lines.push('  topology: hierarchical');
  lines.push('');
  lines.push('# V3 Configuration (.arcanea-flow/config.yaml)');
  lines.push('swarm:');
