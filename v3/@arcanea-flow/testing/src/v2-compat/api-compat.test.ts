...
describe('V2 API Compatibility', () => {
  ...
  describe('Import Path Aliases', () => {
    ...
    it('should resolve arcanea-flow/hive-mind to @arcanea-flow/swarm', () => {
      const resolved = mockRegistry.resolveImport('arcanea-flow/hive-mind');

      expect(resolved).toBe('@arcanea-flow/swarm');
    });

    it('should resolve arcanea-flow/memory to @arcanea-flow/memory', () => {
      const resolved = mockRegistry.resolveImport('arcanea-flow/memory');

      expect(resolved).toBe('@arcanea-flow/memory');
    });
