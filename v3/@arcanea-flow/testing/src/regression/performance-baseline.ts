...
export class PerformanceBaseline {
  ...
  private async simulateStartup(): Promise<void> {
    ...
    await import('@arcanea-flow/shared');
    await import('@arcanea-flow/memory');
  }
  ...
  private async benchmarkEventThroughput(): Promise<number> {
    const { EventBus } = await import('@arcanea-flow/shared');
    const eventBus = new EventBus();

    let count = 0;
    eventBus.subscribe('agent:spawned', () => { count++; });

    const { createAgentSpawnedEvent } = await import('@arcanea-flow/shared');
