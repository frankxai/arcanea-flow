 ...
 * @example
 * ```typescript
 * import { reasoningBankPlugin } from '@arcanea-flow/plugins/examples/ruvector-plugins';
...
export class ReasoningBank {
   ...
   * When set, uses @arcanea-flow/embeddings for high-quality embeddings
   */
  private embeddingProvider: ((text: string) => Promise<Float32Array>) | null = null;

  /**
   * Set external embedding provider from @arcanea-flow/embeddings
   *
   * @example
   * ```typescript
   ...
   * Used when @arcanea-flow/embeddings is not configured
   */
