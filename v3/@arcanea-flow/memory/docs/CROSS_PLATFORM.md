...
The `@arcanea-flow/memory` module provides **universal cross-platform support** through intelligent database provider selection. This ensures your application works seamlessly on Windows, macOS, and Linux without code changes.
...
```typescript
import { createDatabase, getPlatformInfo } from '@arcanea-flow/memory';
...
```typescript
import { createDatabase } from '@arcanea-flow/memory';
...
```typescript
import { createDatabase, getAvailableProviders } from '@arcanea-flow/memory';
...
npm install @arcanea-flow/memory --no-optional
