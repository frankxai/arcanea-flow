...
### 1. Create `@arcanea-flow/providers` Package

A dedicated package for LLM provider implementations:

```
v3/@arcanea-flow/providers/
├── src/
...
Add LLM-specific hooks to `@arcanea-flow/hooks`:
...
### Neutral
- Integration with existing `@arcanea-flow/integration` multi-model-router
...
7. Add LLM hooks to @arcanea-flow/hooks
...
- V3 Multi-Model Router: `v3/@arcanea-flow/integration/src/multi-model-router.ts`
