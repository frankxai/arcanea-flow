# ADR-019: @arcanea-flow/headless Runtime Package
...
Create `@arcanea-flow/headless` package providing:
...
export interface DockerConfig {
  ...
  image: string;  // e.g., 'ghcr.io/ruvnet/arcanea-flow-headless:latest'
...
npx @arcanea-flow/headless serve --port 3001 --sandbox strict

# Execute single prompt
npx @arcanea-flow/headless exec "Fix the bug in auth.ts" --cwd ./project

# Execute from file
npx @arcanea-flow/headless exec --file tasks.txt --parallel 3

# Queue management
npx @arcanea-flow/headless queue add "Refactor utils" --priority high
npx @arcanea-flow/headless queue list
npx @arcanea-flow/headless queue cancel <id>

# Docker mode
npx @arcanea-flow/headless docker start --containers 3
npx @arcanea-flow/headless docker exec "Run tests" --isolated
npx @arcanea-flow/headless docker scale 5

# Monitoring
npx @arcanea-flow/headless status
npx @arcanea-flow/headless metrics --prometheus
```
  ...
  review:
    ...
    steps:
      ...
      - name: Run Claude Review
        ...
        run: |
          npx @arcanea-flow/headless exec \
...
```typescript
import { HeadlessExecutor, TaskQueue } from '@arcanea-flow/headless';
...
spec:
  template:
    spec:
      containers:
      - name: claude
        image: ghcr.io/ruvnet/arcanea-flow-headless:latest
        env:
...
```typescript
import { ContainerExecutor } from '@arcanea-flow/headless';
...
```json
{
  "name": "@arcanea-flow/headless",
  "version": "3.0.0-alpha.1",
  "dependencies": {
    "@arcanea-flow/shared": "^3.0.0-alpha.1",
...
2. **Standalone** - Can be used without other arcanea-flow packages
