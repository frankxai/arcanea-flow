...
npx arcanea-flow@v3alpha hooks transfer export \
...
```bash
# Import from file
npx arcanea-flow@v3alpha hooks transfer import \
...
npx arcanea-flow@v3alpha hooks transfer store list \
  --category routing \
  --language typescript \
  --min-downloads 100

# Publish to store
npx arcanea-flow@v3alpha hooks transfer store publish \
  --input ./patterns.cfp \
...
# Download from store
npx arcanea-flow@v3alpha hooks transfer store download \
...
```typescript
// v3/@arcanea-flow/cli/src/transfer/ipfs-adapter.ts
...
npx arcanea-flow@v3alpha hooks transfer ipfs upload \
  --input ./patterns.cfp \
  --pin pinata \
  --name "my-patterns"

# Download from IPFS
npx arcanea-flow@v3alpha hooks transfer ipfs download \
  --cid bafybeigdyrzt5sfp7udm7hu76uh7y26nf3efuylqabf3oclgtqy55fbzdi \
  --output ./patterns.cfp

# List pinned patterns
npx arcanea-flow@v3alpha hooks transfer ipfs list \
  --service pinata

# Publish to IPNS (mutable name)
npx arcanea-flow@v3alpha hooks transfer ipfs publish \
  --cid bafybeigdyrzt5sfp7udm7hu76uh7y26nf3efuylqabf3oclgtqy55fbzdi \
  --key my-patterns-key

# Resolve IPNS name
npx arcanea-flow@v3alpha hooks transfer ipfs resolve \
  --name my-patterns
```
...
```bash
# Search patterns
npx arcanea-flow@v3alpha hooks transfer store search \
  --query "react hooks optimization" \
  --category routing \
  --min-rating 4.0

# Get pattern info
npx arcanea-flow@v3alpha hooks transfer store info \
  --name react-routing-patterns

# Install pattern
npx arcanea-flow@v3alpha hooks transfer store install \
  --name react-routing-patterns \
  --version ^1.0.0
...
npx arcanea-flow@v3alpha hooks transfer store update \
  --name my-patterns \
  --input ./patterns-v2.cfp

# Rate pattern
npx arcanea-flow@v3alpha hooks transfer store rate \
  --name react-routing-patterns \
  --rating 5 \
...
```json
{
  "transfer": {
    ...
    "store": {
      "enabled": true,
      "registryUrl": "https://patterns.arcanea-flow.dev/registry.json",
      "cacheDir": ".arcanea-flow/patterns"
    },
...
```typescript
// v3/@arcanea-flow/cli/src/mcp-tools/transfer-tools.ts
...
```typescript
// v3/@arcanea-flow/cli/src/mcp-tools/index.ts
...
```
[Warning] [arcanea-flow] mcpServers.arcanea-flow: Windows requires 'cmd /c' wrapper to execute npx
```
...
```json
{
  "mcpServers": {
    "arcanea-flow": {
      "command": "cmd",
      "args": ["/c", "npx", "@arcanea-flow/cli@latest", "mcp", "start"],
      "env": {
...
```json
{
  "mcpServers": {
    "arcanea-flow": {
      "command": "npx",
      "args": ["@arcanea-flow/cli@latest", "mcp", "start"],
      "env": {
        "CLAUDE_FLOW_LOG_LEVEL": "info"
      }
    }
...
npx @arcanea-flow/cli@latest init --force
...
```typescript
// v3/@arcanea-flow/cli/src/init/mcp-generator.ts
...
```bash
# Windows PowerShell
cmd /c npx @arcanea-flow/cli@latest hooks transfer export --output patterns.cfp

# Windows CMD
npx @arcanea-flow/cli@latest hooks transfer export --output patterns.cfp
...
```
v3/@arcanea-flow/cli/src/transfer/store/
...
await store.initialize('arcanea-flow-official');
...
|---------|--------|----------|
| **Memory Store** | ✅ Real | Data persists to `.arcanea-flow/memory/store.json` |
...
| **Registry Discovery** | ⚠️ Demo | Shows "arcanea-flow-official (demo)" source |
...
3. **Test Upload**:
   ```bash
   npx @arcanea-flow/cli hooks transfer store publish \
     -i patterns.cfp \
     -n my-patterns \
     -d "My patterns" \
     -c routing \
...
# Should show: "arcanea-flow-official (demo)" and list 6 official plugins
