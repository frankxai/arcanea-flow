...
arcanea-flow issues list --available

# Claim an issue (as current user/agent)
arcanea-flow issues claim 123

# Release a claim
arcanea-flow issues release 123

# Request handoff to specific agent/human
arcanea-flow issues handoff 123 --to agent:coder-1
arcanea-flow issues handoff 123 --to human:alice

# Update status
arcanea-flow issues status 123 --blocked "Waiting for API spec"
arcanea-flow issues status 123 --review-requested

# View who's working on what
arcanea-flow issues board

# Work stealing commands
arcanea-flow issues stealable              # List stealable issues
arcanea-flow issues steal 123              # Steal an issue
arcanea-flow issues mark-stealable 123     # Mark your claim as stealable
arcanea-flow issues contest 123            # Contest a steal

# Load balancing
arcanea-flow issues load                   # View agent load distribution
arcanea-flow issues load --agent coder-1   # View specific agent load
arcanea-flow issues rebalance              # Trigger swarm rebalancing
arcanea-flow issues rebalance --dry-run    # Preview rebalancing without applying
```
...
- [x] Claim service implemented (`@arcanea-flow/cli/src/services/claim-service.ts`)
...
- [x] CLI commands functional (`@arcanea-flow/cli/src/commands/issues.ts`)
...
|------|-------|-------------|
| `@arcanea-flow/cli/src/services/claim-service.ts` | ~600 | Full claims service with work stealing |
| `@arcanea-flow/cli/src/commands/issues.ts` | ~450 | CLI commands for issue claims |

### CLI Commands Implemented

```bash
# Issue claim commands (10 subcommands)
arcanea-flow issues list          # List all claims
arcanea-flow issues claim <id>    # Claim an issue
arcanea-flow issues release <id>  # Release a claim
arcanea-flow issues handoff       # Request handoff
arcanea-flow issues status <id>   # Get claim status
arcanea-flow issues stealable     # List stealable issues
arcanea-flow issues steal <id>    # Steal an issue
arcanea-flow issues load          # View agent load
arcanea-flow issues rebalance     # Rebalance swarm
arcanea-flow issues board         # Visual claim board
```
...
6. **Persistence**: File-based storage in `.arcanea-flow/claims/claims.json`
