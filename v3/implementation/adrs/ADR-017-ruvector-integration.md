...
The `@arcanea-flow/cli` package requires integration with `ruvector` for advanced code intelligence features:
...
These features are unique to ruvector and complement arcanea-flow's existing capabilities without duplicating functionality already present in `@arcanea-flow/embeddings` or `@arcanea-flow/memory`.

## Decision

Implement ruvector as an **OPTIONAL dependency** with graceful fallback, following the existing patterns in `@arcanea-flow/cli`.
...
```typescript
// v3/@arcanea-flow/cli/src/ruvector/availability.ts
...
```typescript
// v3/@arcanea-flow/cli/src/ruvector/adapters/router-adapter.ts
...
```typescript
// v3/@arcanea-flow/cli/src/ruvector/adapters/ast-adapter.ts
...
```typescript
// v3/@arcanea-flow/cli/src/ruvector/adapters/diff-adapter.ts
...
```typescript
// v3/@arcanea-flow/cli/src/ruvector/adapters/graph-adapter.ts
...
```typescript
// v3/@arcanea-flow/cli/src/commands/route.ts
...
export const routeCommand: Command = {
  ...
  examples: [
    {
      command: 'arcanea-flow route -t "Implement user authentication" --q-learning',
      description: 'Route with Q-Learning model'
    },
    {
      command: 'arcanea-flow route -t "Fix login bug" --coverage-aware',
...
```typescript
// v3/@arcanea-flow/cli/src/commands/analyze.ts
...
const astSubcommand: Command = {
  ...
  examples: [
    { command: 'arcanea-flow analyze ast -p src/', description: 'Analyze src directory' },
    { command: 'arcanea-flow analyze ast -p src/api.ts --complexity', description: 'Get complexity for file' }
  ],
  action: astAction
};
...
const diffSubcommand: Command = {
  ...
  examples: [
    { command: 'arcanea-flow analyze diff --risk', description: 'Analyze current diff with risk' },
    { command: 'arcanea-flow analyze diff --base main --target feature', description: 'Compare branches' },
    { command: 'git diff | arcanea-flow analyze diff --stdin --risk', description: 'Pipe diff from git' }
  ],
  action: diffAction
};
...
const boundariesSubcommand: Command = {
  ...
  examples: [
    { command: 'arcanea-flow analyze boundaries -p src/', description: 'Detect boundaries in src' },
    { command: 'arcanea-flow analyze boundaries -a louvain', description: 'Use Louvain algorithm' }
  ],
...
```typescript
// v3/@arcanea-flow/cli/src/ruvector/errors.ts
...
```typescript
// v3/@arcanea-flow/cli/src/mcp-tools/ruvector-tools.ts
...
```typescript
// v3/@arcanea-flow/cli/src/ruvector/index.ts
...
```typescript
// v3/@arcanea-flow/cli/src/commands/index.ts
...
```typescript
// v3/@arcanea-flow/cli/tests/ruvector/availability.test.ts
...
```typescript
// v3/@arcanea-flow/cli/tests/commands/route.test.ts
...
All cache utilities exported from `@arcanea-flow/cli/ruvector`:

```typescript
// Diff caches
import { clearDiffCache, clearAllDiffCaches } from '@arcanea-flow/cli/ruvector';

// Graph caches
import { clearGraphCaches, getGraphCacheStats } from '@arcanea-flow/cli/ruvector';

// Coverage caches
import { clearCoverageCache, getCoverageCacheStats } from '@arcanea-flow/cli/ruvector';
```
...
**Changes Made:**
- `@arcanea-flow/cli/src/commands/index.ts` - Refactored to use dynamic imports
...
**Changes Made:**
- `@arcanea-flow/memory/src/agentdb-adapter.ts` - Optimized bulk methods
...
|---------|--------|-------|
| `arcanea-flow` | 3.0.0-alpha.17 | 3.0.0-alpha.18 |
| `@arcanea-flow/cli` | 3.0.0-alpha.24 | 3.0.0-alpha.25 |
| `@arcanea-flow/mcp` | 3.0.0-alpha.7 | 3.0.0-alpha.8 |

### Usage Examples

**Lazy Command Loading:**
```typescript
import { getCommandAsync, loadAllCommands } from '@arcanea-flow/cli';
...
```typescript
import { UnifiedMemoryService } from '@arcanea-flow/memory';
...
- `arcanea-flow@3.0.0-alpha.18`
- `@arcanea-flow/cli@3.0.0-alpha.25`
- `@arcanea-flow/mcp@3.0.0-alpha.8`
...
```
✓ @arcanea-flow/cli build passed
✓ @arcanea-flow/mcp build passed
✓ @arcanea-flow/memory build passed
```
