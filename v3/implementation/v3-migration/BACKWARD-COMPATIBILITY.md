...
```typescript
// v3/@arcanea-flow/shared/src/compat/v2-aliases.ts
...
export { UnifiedSwarmCoordinator as HiveMind } from '@arcanea-flow/swarm';
export { UnifiedSwarmCoordinator as SwarmCoordinator } from '@arcanea-flow/swarm';
export { UnifiedMemoryService as MemoryManager } from '@arcanea-flow/memory';
```
...
```typescript
// v3/@arcanea-flow/cli/src/compat/v2-commands.ts
...
```typescript
// v3/@arcanea-flow/cli/src/commands/migrate.ts
export async function migrateConfig(v2ConfigPath: string): Promise<void> {
  ...
  const v3Config = {
    ...
    memory: {
      backend: 'hybrid',
      sqlite: {
        path: v2Config.memory?.path || './.arcanea-flow/memory.db'
      },
      agentdb: {
        enableHNSW: true,
        dimensions: 384
  ...
  await saveYaml('.arcanea-flow/config.yaml', v3Config);
}
...
```typescript
// v3/@arcanea-flow/memory/src/migration.ts
