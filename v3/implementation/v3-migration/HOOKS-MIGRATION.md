...
### V3 Hook Registry
```typescript
// V3: @arcanea-flow/shared/src/hooks/registry.ts
class HookRegistry {
...
import { HookRegistry } from '@arcanea-flow/shared/hooks';
...
```bash
# V2 Commands
npx arcanea-flow hooks pre-task --description "Task" --task-id ID
npx arcanea-flow hooks post-task --task-id ID
npx arcanea-flow hooks pre-edit --file path
npx arcanea-flow hooks post-edit --file path --success true
npx arcanea-flow hooks pre-command --command "npm test"
npx arcanea-flow hooks post-command --command "npm test" --success true
npx arcanea-flow hooks session-end
npx arcanea-flow hooks session-restore --session-id latest
npx arcanea-flow hooks notify --message "Done" --level success
```

### V3 Hooks CLI
```bash
# V3 Commands (implemented)
npx arcanea-flow hooks pre-edit <filePath>
npx arcanea-flow hooks post-edit <filePath> --success true
npx arcanea-flow hooks pre-command "<command>"
npx arcanea-flow hooks post-command "<command>" --success true
npx arcanea-flow hooks route "<task description>"
npx arcanea-flow hooks explain "<task description>"
